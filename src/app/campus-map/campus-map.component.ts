...
export class CampusMapComponent implements OnInit, AfterViewInit, OnDestroy {
  @Input()
  eyebrow = 'Campus di Viale delle Scienze';
  @Input()
  heading = 'Esplora gli edifici';
  @Input()
  hint = 'Seleziona un edificio sulla mappa';
  ...
  private loadBuildings(): void {
    this.request = forkJoin({
      ...
      }>(`${environment.apiUrl}/buildings`),
      spaces: allPages<MapSpace>(
        this.http,
        `${environment.apiUrl}/spaces?size=100`,
      ),
    }).subscribe({
      next: (response) => {
        ...
        this.buildings = response.buildings.data
          ...
          .map((building) => ({
            ...
            spaces: spacesByBuilding.get(building.id) ?? [],
          }));
        this.loading = false;
        this.error = '';
      ...
      error: () => {
        ...
        this.buildings = [];
        this.error =
          'Impossibile caricare gli edifici e gli spazi della mappa.';
        this.changeDetector.markForCheck();
      },
    });
  }
  ...
  private addBuildingMarkers(): void {
    ...
    for (const building of this.buildings) {
      ...
      marker.on('popupopen', (event) => {
        ...
        element
          ?.querySelectorAll<HTMLAnchorElement>('a[data-space-id]')
          .forEach((link) => {
            link.onclick = (click) => {
    ...
    if (this.buildings.length) {
      this.map?.fitBounds(
        L.latLngBounds(
          this.buildings.map(
            (building) =>
              [building.latitude, building.longitude] as [number, number],
          ),
        ),
  ...
  private popupHtml(building: MapBuilding): string {
    ...
    const types = [
      ...new Set(building.spaces.map((space) => this.typeLabel(space.type))),
    ].join(', ');
    const links = building.spaces
      .map(
        (space) =>
          `<li><a href="/spaces/${Number(space.id)}" data-space-id="${Number(space.id)}">${this.escapeHtml(space.name)}</a></li>`,
      )
      .join('');
