  ...
  updateStatus(status: string): void {
    ...
    this.http
      .patch<{
        ...
        `${environment.apiUrl}/admin/reports/${this.selectedReport.id}/status`,
        { status: apiStatus },
      )
      .subscribe({
        next: () => {
          if (this.selectedReport) {
            ...
            this.selectedReport.statusLabel =
              status === 'in-progress' ? 'In lavorazione' : 'Risolta';
          }
        ...
        error: (error: HttpErrorResponse) => {
          ...
          this.statusError =
            typeof message === 'string'
              ? message
              : `Salvataggio non riuscito (HTTP ${error.status}).`;
          this.changeDetector.markForCheck();
        },
      });
  }
      ...
      .subscribe({
        next: (response) => {
          const reports = response.data.map((report) => ({
            ...
            date: new Date(report.createdAt).toLocaleDateString('it-IT', {
              timeZone: 'Europe/Rome',
            }),
            reporter: report.authorEmail,
