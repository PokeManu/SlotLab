...
describe('SpaceDetailPage', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideRouter([]), { provide: ActivatedRoute, useValue: { snapshot: { paramMap: convertToParamMap({ id: '1' }) } } }, provideHttpClient(), provideHttpClientTesting()],
    });
    ...
    http.match(request => request.url.includes('/availability')).forEach(request => request.flush({ data: [] }));
  });
  ...
  function reloadAvailability() {
    ...
    http.expectOne('/api/v1/spaces/1').flush({ data: {
      ...
      type: 'study_room', capacity: 10, accessible: true, status: 'active', services: [],
    } });
  ...
  it('mostra i posti residui della prima fascia prenotabile e si aggiorna al rientro', async () => {
    reloadAvailability().flush({ data: [
      { bookable: false, availableSeats: 0, startTime: '09:00', endTime: '10:00' },
      { bookable: true, availableSeats: 3, startTime: '11:00', endTime: '12:00' },
      { bookable: true, availableSeats: 8, startTime: '14:00', endTime: '15:00' },
    ] });
    await fixture.whenStable();
    ...
    expect(fixture.nativeElement.textContent).toContain('Capienza totale: 10 posti');
    ...
    http.expectOne(request => request.url.includes('/availability')).flush({ data: [
      { bookable: true, availableSeats: 1, startTime: '11:00', endTime: '12:00' },
    ] });
    await fixture.whenStable();
  ...
  it('distingue errore di caricamento da nessuna fascia prenotabile', async () => {
    reloadAvailability().flush({ data: [] });
    await fixture.whenStable();
    expect(fixture.nativeElement.textContent).toContain('Nessuna fascia prenotabile oggi');
    reloadAvailability().flush({}, { status: 500, statusText: 'Server Error' });
    await fixture.whenStable();
    expect(component.availableSeats).toBeNull();
    expect(fixture.nativeElement.textContent).toContain('Impossibile verificare');
  ...
  it('associa testi italiani e icone distinte ai cinque servizi reali', () => {
    ...
    fixture.detectChanges();
    const cards = Array.from(fixture.nativeElement.querySelectorAll('.service-card')) as HTMLElement[];
    expect(cards.map(card => card.textContent?.trim())).toEqual(['Wi-Fi', 'Prese elettriche', 'Proiettore', 'Computer', 'Aria condizionata']);
    expect(cards.map(card => (card.querySelector('ion-icon') as HTMLElement & { name: string })?.name)).toEqual(['wifi-outline', 'flash-outline', 'easel-outline', 'desktop-outline', 'snow-outline']);
  });
