...
import { ChangeDetectorRef, Component, EventEmitter, Input, Output, OnChanges, OnDestroy, inject } from '@angular/core';
...
export class ReportDetailComponent implements OnChanges, OnDestroy {
  ...
  ngOnChanges(): void {
    ...
    this.photoRequest = this.http.get(`${environment.apiUrl}/admin/reports/${this.selectedReport.id}/photo`, { responseType: 'blob' }).subscribe({
      next: photo => {
