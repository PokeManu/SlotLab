...
export class AdminStatisticsPage implements OnInit, OnDestroy {
  ...
  get chartTicks(): DailyBooking[] {
    ...
    return Array.from(
      { length: 7 },
      (_, index) => this.dailyBookings[Math.round(index * step)],
    );
  }
  ...
  reportStatusLabel(status: string): string {
    return (
      { open: 'Aperte', in_progress: 'In lavorazione', resolved: 'Risolte' }[
        status
      ] ?? status
    );
  }
  loadStatistics(): void {
    ...
    this.request = this.http
      .get<StatisticsResponse>(`${environment.apiUrl}/admin/statistics${query}`)
      .pipe(
        finalize(() => {
  ...
  private updateView(data: StatisticsData): void {
    ...
    this.metrics = [
      {
        ...
        trend: 'gruppi completati',
      },
      {
        label: 'Tasso di utilizzo',
        value: this.rate(historyHasIssues ? null : data.utilizationRate),
        trend: historyHasIssues
          ? 'posti storici non determinabili'
          : 'presenze / posti offerti',
      },
      {
        label: 'Check-in completati',
        value: this.rate(data.checkInRate),
        trend: 'presenze / partecipanti',
      },
    ];
    const max =
      Math.ceil(Math.max(1, ...data.daily.map((item) => item.bookings)) / 4) *
      4;
    ...
    this.usageCategories = data.usage.flatMap((item) =>
      labels[item.type]
    ...
    if (!data.history.complete) {
      const since = data.history.trackingStartedAt
        ? new Intl.DateTimeFormat('it-IT', {
            ...
            dateStyle: 'short',
            timeStyle: 'short',
          }).format(new Date(data.history.trackingStartedAt))
        : 'questa installazione';
      historyNotes.push(
        `i dati precedenti a ${since} potrebbero mancare; gli indicatori si riferiscono alle sole fasce documentate`,
      );
    }
    if (data.history.excludedBookings > 0) {
      historyNotes.push(
        `${data.history.excludedBookings} prenotazioni escluse`,
      );
    }
    if (historyHasIssues) {
      historyNotes.push(
        `${ambiguousOccurrences} fasce ambigue e ${gapOccurrences} fasce mancanti; i posti offerti non sono determinabili per queste fasce`,
      );
    }
    this.historyMessage = historyNotes.length
      ? `Storia parziale: ${historyNotes.join('; ')}.`
      : '';
  }
  private normalise(data: StatisticsData): StatisticsData {
    return {
      ...
      daily: data.daily ?? [],
      usage: data.usage ?? [],
      mostBookedSpaces: data.mostBookedSpaces ?? [],
      mostUsedSlots: data.mostUsedSlots ?? [],
  ...
  private emptyMetrics(): StatisticMetric[] {
    return [
      { label: 'Prenotazioni', value: '0', trend: 'nessun dato caricato' },
      {
        label: 'Tasso di utilizzo',
        value: 'N/D',
        trend: 'nessun posto offerto',
      },
      {
        label: 'Check-in completati',
        value: 'N/D',
        trend: 'nessun partecipante',
      },
    ];
  }
  ...
  private makeUsageGradient(categories: UsageCategory[]): string {
    if (!categories.length || categories.every((item) => item.percentage <= 0))
