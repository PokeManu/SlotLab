...
function statistics(overrides: Record<string, unknown> = {}) {
  return {
    dateFrom: '2026-09-01', dateTo: '2026-09-08', bookings: 2, participants: 3,
    presences: 2, absences: 1, offeredCapacity: 20, utilizationRate: 10, checkInRate: 67,
    daily: [{ date: '2026-09-02', bookings: 2 }],
    usage: [{ type: 'study_room', presences: 2, percentage: 100 }],
    mostBookedSpaces: [{ spaceId: 1, spaceName: 'Aula A1', bookings: 2, participants: 3, presences: 2 }],
    mostUsedSlots: [{ startTime: '10:00', endTime: '12:00', bookings: 2, participants: 3, presences: 2 }],
...
describe('AdminStatisticsPage', () => {
  ...
  it('mostra N/D per rapporti con denominatore zero e avvisa per storia incompleta', () => {
    ...
    request.flush({ data: statistics({ utilizationRate: null, checkInRate: null, history: { trackingStartedAt: '2026-09-05T10:00:00.000Z', complete: false, excludedBookings: 2 } }) });
    fixture.detectChanges();
  ...
  it('rende N/D l’utilizzo quando il backend segnala fasce ambigue o mancanti', () => {
    ...
    request.flush({ data: statistics({
      utilizationRate: 25,
      history: { trackingStartedAt: '2026-09-01T10:00:00.000Z', complete: true, excludedBookings: 0, ambiguousOccurrences: 1, gapOccurrences: 2 },
    }) });
    fixture.detectChanges();

    expect(component.metrics[1].value).toBe('N/D');
    expect(component.historyMessage).toContain('1 fasce ambigue e 2 fasce mancanti');
  });
  ...
  it('applica una coppia di date selezionata e ricarica al rientro', () => {
    ...
    const changed = http.expectOne('/api/v1/admin/statistics?dateFrom=2026-08-01&dateTo=2026-08-31');
    changed.flush({ data: statistics({ dateFrom: '2026-08-01', dateTo: '2026-08-31' }) });
    fixture.detectChanges();
    ...
    component.ionViewDidEnter();
    http.expectOne('/api/v1/admin/statistics?dateFrom=2026-08-01&dateTo=2026-08-31').flush({ data: statistics({ dateFrom: '2026-08-01', dateTo: '2026-08-31' }) });
  });
  ...
  it('rifiuta un intervallo invertito senza inviare la richiesta', () => {
    ...
    http.expectNone('/api/v1/admin/statistics?dateFrom=2026-09-30&dateTo=2026-09-01');
  });
  ...
  it('azzera i dati precedenti quando la richiesta fallisce', () => {
    ...
    failed.flush({ error: { error: { code: 'INTERNAL_ERROR' } } }, { status: 500, statusText: 'Server Error' });
    fixture.detectChanges();
