...
describe('NotificationsPage', () => {
  ...
  it('aggiorna immediatamente lo stile dopo la lettura e gestisce gli errori', async () => {
    ...
    http
      .expectOne('/api/v1/notifications/1/read')
      .flush({ data: { read: true } });
    await fixture.whenStable();
    expect(
      fixture.nativeElement.querySelector('.notification-card--read'),
    ).not.toBeNull();
    expect(
      fixture.nativeElement.querySelector('.notification-card__context')
        .textContent,
    ).toContain('Segnalazione');
    component.loadNotifications();
    http
      .expectOne('/api/v1/notifications')
      ...
      data: [
        {
          ...
          message: 'Risolta',
          createdAt: '2026-09-07T17:52:18.250Z',
          read: false,
        },
      ],
    });
    expect(component.notificationGroups[0].notifications[0].title).toBe(
