...
describe('UserBookingDetailPage', () => {
  ...
  it('aggiunge tramite email e rimuove usando ID partecipazione, non ID utente', async () => {
    ...
    const add = http.expectOne({
      method: 'POST',
      url: '/api/v1/bookings/7/participants',
    });
    ...
    http
      .expectOne({
        method: 'DELETE',
        url: '/api/v1/bookings/7/participants/90',
      })
      .flush(null);
    http
      .expectOne('/api/v1/bookings/7')
      .flush({ data: { ...booking, participants: [organizer] } });
    await fixture.whenStable();
