...
export class UserBookingDetailPage implements OnInit {
  ...
  ionViewWillEnter(): void {
    if (this.hasEntered) this.load();
    this.hasEntered = true;
  }
  isOrganizer(): boolean {
    return (
      this.booking()?.participants.some(
        (p) =>
          p.id === this.auth.user()?.id && p.participantRole === 'organizer',
      ) ?? false
    );
  }
  load(): void {
    ...
    if (!id || !/^[1-9]\d*$/.test(id)) {
      this.error.set('Prenotazione non valida.');
      return;
    }
    this.loadRequest = this.http
      ...
      .subscribe({
        next: (response) => this.booking.set(response.data),
  ...
  add(): void {
    ...
    this.http
      .post(
        `${environment.apiUrl}/bookings/${this.booking()!.id}/participants`,
  ...
  remove(person: Participant): void {
    ...
    this.http
      .delete(
        `${environment.apiUrl}/bookings/${this.booking()!.id}/participants/${person.participantId}`,
      )
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          this.busy.set(false);
          if (person.id === this.auth.user()?.id)
