...
describe('BookingPage', () => {
  ...
  it('mostra le fasce ricevute dal server', async () => {
    ...
    http
      .expectOne('/api/v1/spaces/1/availability?date=' + component.selectedDate)
      .flush({
        data: [
          {
            availabilityId: 4,
    ...
    fixture.detectChanges();
    expect(
      fixture.nativeElement.querySelector('.time-slot')?.textContent,
  ...
  it('al rientro sostituisce la fascia ritirata con quella nuova', async () => {
    ...
    expect(
      fixture.nativeElement.querySelector('.time-slots').textContent,
    ).toContain('14:00-16:00');
    expect(
      fixture.nativeElement.querySelector('.time-slots').textContent,
  ...
  it('annulla la richiesta precedente quando cambia la data e distingue errore da elenco vuoto', async () => {
    ...
    const old = http.expectOne(
      '/api/v1/spaces/1/availability?date=' + component.selectedDate,
    );
    component.selectedDate = '2026-09-15';
    (component as any).loadAvailability('1');
    expect(old.cancelled).toBe(true);
    http
      .expectOne('/api/v1/spaces/1/availability?date=2026-09-15')
      .flush({ data: [] });
    await fixture.whenStable();
  ...
  it('aggiunge un input per ogni ospite, conserva gli altri e rispetta i posti liberi', () => {
    ...
    expect(
      fixture.nativeElement.querySelectorAll('input[type=email]').length,
    ).toBe(2);
