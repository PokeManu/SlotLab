  ...
  ngOnInit(): void {
    ...
    allPages<{ date: string; startTime: string; spaceName: string; building: string; participantCount: number; status: string }>(this.http, `${environment.apiUrl}/admin/bookings?size=5`).subscribe({
      next: response => {
        ...
        this.bookings = response.data.filter(item => item.status === 'confirmed' && `${item.date} ${item.startTime}` > current)
          .sort((a,b) => `${a.date} ${a.startTime}`.localeCompare(`${b.date} ${b.startTime}`)).slice(0,5)
          .map(item => ({ time: `${item.date} ${item.startTime}`, name: item.spaceName, building: item.building, details: `${item.participantCount} partecipanti`, status: 'Confermata' }));
        this.changeDetector.markForCheck();
      },
