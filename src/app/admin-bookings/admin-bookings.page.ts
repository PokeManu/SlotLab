...
export class AdminBookingsPage implements OnInit, OnDestroy {
  ...
  loadBookings(): void {
    ...
    const fetchPage = (page: number) =>
      this.http.get<BookingResponse>(
        `${environment.apiUrl}/admin/bookings?page=${page}&size=100`,
      );
    this.request = fetchPage(1)
      .pipe(
        expand((response) =>
          response.pagination.page < response.pagination.totalPages
            ? fetchPage(response.pagination.page + 1)
            : EMPTY,
        ),
        reduce(
          (rows: ApiBooking[], response) => rows.concat(response.data),
          [],
        ),
      )
      .subscribe({
        next: (rows) => {
          this.bookings = rows.map((booking) => ({
            ...
            code: String(booking.id),
            startTime: booking.startTime,
            endTime: booking.endTime,
            space: booking.spaceName,
            bookedBy: booking.organizerName ?? '—',
            participants: booking.participantCount,
            status:
              booking.status === 'completed' ? 'Completata' : 'Confermata',
          ...
          this.selectedBooking =
            this.bookings.find((booking) => booking.code === selectedCode) ??
            null;
  ...
  get availableDates(): string[] {
    return [
      ...new Set(this.bookings.map((booking) => booking.dateValue)),
    ].sort();
  }
  ...
  get filteredBookings(): AdminBooking[] {
    ...
    return this.bookings.filter((booking) => {
      ...
      const matchesSpace =
        this.selectedSpace.length === 0 || booking.space === this.selectedSpace;
      const matchesStatus =
        this.selectedStatus.length === 0 ||
        booking.status === this.selectedStatus;
      return matchesSearch && matchesDate && matchesSpace && matchesStatus;
    });
  }
  ...
  get visibleBookings(): AdminBooking[] {
    ...
    return this.filteredBookings.slice(firstIndex, firstIndex + this.pageSize);
  }
