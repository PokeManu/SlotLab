...
export class AccountPage {
  ...
  get passwordErrors(): string[] {
    return this.mode === 'password'
      ? passwordValidationMessages(this.newPassword)
      : [];
  }
  get title() {
  ...
  submit() {
    ...
    if (this.mode !== 'forgot' && !this.currentPassword) {
      this.error.set('Inserisci la password corrente.');
      return;
    }
    if (this.mode === 'delete' && !this.confirmed) {
      this.error.set('Conferma la cancellazione definitiva.');
      return;
    }
    if (
      this.mode === 'password' &&
      (this.newPassword !== this.confirmation || this.passwordErrors.length)
    )
      return;
    this.busy.set(true);
    const request =
      this.mode === 'forgot'
        ? this.http.post<void>(`${environment.apiUrl}/auth/forgot-password`, {
    ...
    request.pipe(finalize(() => this.busy.set(false))).subscribe({
      ...
      error: (error) => {
        this.error.set(
          error.error?.error?.code === 'CURRENT_PASSWORD_INVALID'
            ? 'La password corrente non è corretta.'
            : error.error?.error?.code === 'INVALID_PASSWORD_FORMAT'
              ? 'La nuova password non rispetta i requisiti.'
              : error.status === 429
