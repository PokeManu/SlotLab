  ...
  template: ` <ion-content>
    <main class="check-in">
      <section class="check-in__card" aria-labelledby="check-in-title">
        <div class="check-in__brand">
          <img src="assets/branding/slotlab-logo.png" alt="SlotLab" />
        </div>
        <p class="check-in__eyebrow">Presenze</p>
        <h1 id="check-in-title">Check-in</h1>
        @if (validId) {
          <p class="check-in__intro">
            Spazio {{ spaceId }} · Verifica la tua presenza per la prenotazione.
          </p>
          ...
          <button type="button" (click)="verify()" [disabled]="loading()">
            {{ loading() ? 'Verifica in corso…' : 'Verifica check-in' }}
          </button>
        } @else {
...
export class CheckInEntryPage {
  ...
  get validId() {
    return /^[1-9]\d*$/.test(this.spaceId);
  }
  verify(): void {
    ...
    this.http
      ...
      }>(`${environment.apiUrl}/spaces/${this.spaceId}/check-in`, {})
