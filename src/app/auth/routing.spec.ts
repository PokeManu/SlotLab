...
describe('Route reali e destinazioni dopo accesso', () => {
  ...
  beforeEach(() => {
    const testRoutes = routes.map((route) =>
      route.data?.['access']
        ? { ...route, loadComponent: () => Promise.resolve(PrivatePage) }
        : route,
    );
  ...
  function login(role: UserRole) {
    TestBed.inject(Auth)
      .login({ email: 'anna@example.test', password: 'TestPassword2026!' })
      .subscribe();
    respondLogin(role);
  }
  ...
  for (const role of ['user', 'admin'] as const) {
    it(`separa tutte le pagine per ${role}, incluso admin/bookings e accessi diretti`, async () => {
      ...
      for (const route of routes.filter((route) => route.data?.['access'])) {
        ...
        await harness.navigateByUrl(url);
        const allowed =
          route.data!['access'] === role ||
          route.data!['access'] === 'authenticated';
        expect(router.url).toBe(
          allowed ? url : role === 'admin' ? '/admin' : '/home',
        );
      }
  ...
  it('rifiuta ritorni esterni, pubblici, sconosciuti, outlet secondari e ruoli incompatibili', () => {
    ...
    expect(
      String(
        returnDestination(router, '/spaces/12?date=2026-09-07#slots', 'user'),
      ),
  ...
  it('conserva il QR anche passando da registrazione, poi il login torna allo spazio richiesto', async () => {
    ...
    const registerUrl = router.createUrlTree(['/register'], {
      queryParams: router.parseUrl(router.url).queryParams,
    });
    const register = await harness.navigateByUrl(
      router.serializeUrl(registerUrl),
      AccessPage,
    );
    register.firstName = 'Anna';
    register.lastName = 'Rossi';
    register.email = 'anna@example.test';
    register.password = 'TestPassword2026!';
    register.passwordConfirmation = 'TestPassword2026!';
    register.submit();
    http.expectOne('/api/v1/auth/register').flush({ data: { id: 1 } });
    await harness.fixture.whenStable();
    expect(router.parseUrl(router.url).queryParams).toEqual({
      returnUrl: '/check-in/12',
      registered: '1',
    });
    const page = harness.routeDebugElement!.componentInstance as AccessPage;
    page.email = 'anna@example.test';
    page.password = 'TestPassword2026!';
    page.submit();
    respondLogin('user');
    await harness.fixture.whenStable();
  ...
  it('un login admin ignora un ritorno QR riservato agli utenti', async () => {
    ...
    const page = await harness.navigateByUrl(
      '/login?returnUrl=%2Fcheck-in%2F12',
      AccessPage,
    );
    page.email = 'anna@example.test';
    page.password = 'TestPassword2026!';
  ...
  it('un form conservato usa la destinazione corrente anche con snapshot vecchio', async () => {
    ...
    vi.spyOn(router, 'url', 'get').mockReturnValue(
      '/login?returnUrl=%2Fcheck-in%2F12&registered=1',
    );
  ...
  it('le barre condivise mostrano soltanto i collegamenti del ruolo corrente', () => {
    ...
    for (const fixture of [topbar, mobile]) {
      ...
      fixture.detectChanges();
      expect(
        fixture.nativeElement.querySelector('a[href="/admin/bookings"]'),
      ).toBeNull();
      login('admin');
    }
  });
