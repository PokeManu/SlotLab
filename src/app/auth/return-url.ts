...
export function returnDestination(
  ...
  try {
    ...
    if (
      !primary ||
      Object.keys(tree.root.children).length !== 1 ||
      Object.keys(primary.children).length
    )
      return null;
    ...
    if (
      segments.some(
        (segment) =>
          Object.keys(segment.parameters).length ||
          !/^[\w-]+$/.test(segment.path),
      )
    )
      return null;
    ...
    if (
      ...
      return null;
    if (
      segments[0].path === 'check-in' &&
      !/^[1-9]\d*$/.test(segments[1]?.path ?? '')
    )
      return null;
    return tree;
  } catch {
...
export function loginDestination(
  ...
  return router.createUrlTree(['/login'], {
    queryParams: destination
      ? { returnUrl: router.serializeUrl(destination) }
      : {},
  });
}
