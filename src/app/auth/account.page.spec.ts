...
describe('Gestione credenziali e account', () => {
  ...
  function login() {
    TestBed.inject(Auth)
      .login({ email: 'test@example.test', password: 'TestPassword2026!' })
      .subscribe();
    http.expectOne('/api/v1/auth/login').flush({
      data: {
        accessToken: 'token',
        user: { id: 1, firstName: 'Test', lastName: 'User', role: 'user' },
      },
    });
  }
  afterEach(() => http.verify());
  it('mostra il logo ufficiale nei flussi account', () => {
    const fixture = create('forgot');
  ...
  it('cambio password verifica conferma e conserva stato in caso di password errata', () => {
    const page = create('password').componentInstance;
    login();
    page.currentPassword = 'TestPassword2026!';
    page.newPassword = 'Changed2026!';
    page.confirmation = 'different';
    page.submit();
    http.expectNone('/api/v1/users/me/password');
    page.confirmation = page.newPassword;
    page.submit();
    http
    ...
    expect(page.error()).toContain('corrente');
    expect(TestBed.inject(Auth).user()).not.toBeNull();
    page.submit();
    http
      .expectOne('/api/v1/users/me/password')
      .flush(null, { status: 204, statusText: 'No Content' });
    expect(TestBed.inject(Auth).accessToken).toBeNull();
  ...
  it('eliminazione richiede conferma e password; errore rete conserva account e consente retry', () => {
    const page = create('delete').componentInstance;
    login();
    page.currentPassword = 'TestPassword2026!';
    page.submit();
  ...
  it('aspetta un rinnovo in corso prima di cambiare password e impedisce nuovi login concorrenti', () => {
    ...
    page.currentPassword = 'TestPassword2026!';
    page.newPassword = page.confirmation = 'Changed2026!';
    page.submit();
    http.expectNone('/api/v1/users/me/password');
    auth
      .login({ email: 'other@example.test', password: 'Other2026!' })
      .subscribe({ error: () => {} });
    http.expectNone('/api/v1/auth/login');
    http
      .expectOne('/api/v1/auth/refresh')
      .flush({ data: { accessToken: 'fresh-token' } });
