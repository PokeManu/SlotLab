...
function notificationId(value) {
  if (!/^[1-9][0-9]*$/.test(String(value)))
    throw Object.assign(new Error("Identificativo notifica non valido."), {
      status: 400,
...
router.patch("/read-all", async (request, response) => {
  await run(
    ...
    [new Date().toISOString(), request.user.id],
  );
...
router.patch("/:notificationId/read", async (request, response) => {
  ...
  const existing = await new Promise((resolve, reject) =>
    getDatabase().get(
      "SELECT id, read_at AS readAt FROM notifications WHERE id = ? AND user_id = ?;",
      [id, request.user.id],
      (error, row) => (error ? reject(error) : resolve(row)),
    ),
  );
  if (!existing)
    throw Object.assign(new Error("La notifica richiesta non esiste."), {
      status: 404,
  ...
  if (existing.readAt === null)
    await run(
      ...
      [new Date().toISOString(), id, request.user.id],
    );
