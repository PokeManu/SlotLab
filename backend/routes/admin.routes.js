...
async function queuePhotos(db, photos) {
  for (const photo of photos)
    await db.run(
      "INSERT OR IGNORE INTO file_deletions(filename,created_at) VALUES(?,?)",
      [photoName(photo.photo_path), new Date().toISOString()],
    );
}
async function cleanupPhotos() {
  await cleanDeletedFiles().catch(() =>
    console.error("Pulizia fotografie da riprovare."),
  );
}
async function prepareSpaceDeletion(db, id) {
  const users = await db.all(
    `SELECT DISTINCT bp.user_id AS id FROM bookings b
    JOIN availabilities a ON a.id=b.availability_id JOIN booking_participants bp ON bp.booking_id=b.id
    WHERE b.space_id=? AND b.status='confirmed' AND b.date || ' ' || a.start_time > ?`,
    [id, romeDateTime()],
  );
  for (const user of users)
    await db.run(
      "INSERT INTO notifications(user_id,type,title,message,created_at) VALUES(?,'space_unavailable','Prenotazione cancellata','Lo spazio della prenotazione è stato eliminato.',?)",
      [user.id, new Date().toISOString()],
    );
  await queuePhotos(
    db,
    await db.all(
      "SELECT photo_path FROM reports WHERE space_id=? AND photo_path IS NOT NULL",
      [id],
    ),
  );
}
...
async function rejectIfBookingInProgress(
  db,
  spaceId,
  now = new Date(),
  interval = null,
  ...
  const parameters = [spaceId, local, local];
  const overlap = interval
    ? " AND b.date = ? AND a.start_time < ? AND a.end_time > ?"
    : "";
  if (interval)
    parameters.push(interval.date, interval.endTime, interval.startTime);
  const booking = await db.get(
    `SELECT b.id FROM bookings b JOIN availabilities a ON a.id = b.availability_id
      WHERE b.space_id = ? AND b.status = 'confirmed'
        AND b.date || ' ' || a.start_time <= ? AND b.date || ' ' || a.end_time > ?${overlap}
      LIMIT 1;`,
    parameters,
  );
  if (booking)
    throw Object.assign(
      new Error(
        "L’operazione non è consentita durante una prenotazione in corso.",
      ),
      { status: 409, code: "BOOKING_IN_PROGRESS" },
    );
}
async function cancelBookings(db, bookings, createdAt, message) {
  for (const booking of bookings) {
    const participants = await db.all(
      "SELECT user_id FROM booking_participants WHERE booking_id=?",
      [booking.id],
    );
    for (const participant of participants) {
      await db.run(
        `INSERT INTO notifications(user_id,type,title,message,created_at)
        VALUES(?,'space_unavailable','Prenotazione cancellata',?,?)`,
        [participant.user_id, message, createdAt],
      );
    }
...
async function reconcileAvailability(db, spaceId, body, replacedId, now) {
  ...
  const bookings = await db.all(
    `SELECT b.id,b.availability_id,b.date,a.start_time,a.end_time
    FROM bookings b JOIN availabilities a ON a.id=b.availability_id
    WHERE b.space_id=? AND b.date || ' ' || a.end_time > ?`,
    [spaceId, local],
  );
  const incompatible = [];
  for (const booking of bookings) {
    ...
    const applies =
      booking.date >= body.validFrom &&
      booking.date <= body.validUntil &&
      weekday === Number(body.weekday);
    const same =
      applies &&
      booking.start_time === body.startTime &&
      booking.end_time === body.endTime;
    const overlaps =
      applies &&
      booking.start_time < body.endTime &&
      booking.end_time > body.startTime;
    if (!same && (booking.availability_id === replacedId || overlaps)) {
      if (`${booking.date} ${booking.start_time}` <= local) {
        if (overlaps)
          throw Object.assign(
            new Error(
              "La nuova fascia si sovrappone a una prenotazione già iniziata.",
            ),
            { status: 409, code: "AVAILABILITY_OVERLAP" },
          );
  ...
  await cancelBookings(
    ...
    incompatible,
    now.toISOString(),
    "La prenotazione è stata cancellata perché la configurazione della fascia è cambiata.",
  );
}
router.post("/announcements", async (request, response) => {
  ...
  if (
    ...
    throw Object.assign(
      new Error("Inserisci titolo e messaggio dell’avviso."),
  ...
  if (message.length > MAX_ANNOUNCEMENT_MESSAGE_LENGTH) {
    throw Object.assign(
      new Error(
        `Il messaggio non può superare ${MAX_ANNOUNCEMENT_MESSAGE_LENGTH} caratteri.`,
      ),
      { status: 400, code: "VALIDATION_ERROR" },
    );
  }
  ...
  const data = await transaction(async (db) => {
    ...
    return {
      id: announcement.lastId,
      title,
      message,
      createdAt,
      recipientCount: delivered.changes,
    };
  });
...
router.get("/buildings", async (request, response) => {
  response.json({
    data: await all(
      "SELECT id, number, name, address, latitude, longitude FROM buildings ORDER BY number, id;",
    ),
  });
});
router.get("/buildings/:buildingId", async (request, response) => {
  ...
  if (!Number.isInteger(id) || id < 1)
    throw Object.assign(new Error("Identificativo edificio non valido."), {
      status: 400,
      code: "INVALID_BUILDING_ID",
    });
  const row = await new Promise((resolve, reject) =>
    getDatabase().get(
      "SELECT id, number, name, address, latitude, longitude FROM buildings WHERE id = ?;",
      [id],
      (error, value) => (error ? reject(error) : resolve(value)),
    ),
  );
  if (!row)
    throw Object.assign(new Error("L’edificio richiesto non esiste."), {
      status: 404,
...
router.post("/buildings", async (request, response) => {
  ...
  if (
    ...
    throw Object.assign(new Error("I dati dell’edificio non sono validi."), {
      status: 400,
  ...
  try {
    const created = await new Promise((resolve, reject) =>
      getDatabase().run(
        ...
        [number, body.name.trim(), body.address.trim(), latitude, longitude],
    ...
    if (cause.code === "SQLITE_CONSTRAINT")
      throw Object.assign(new Error("Il numero ufficiale è già utilizzato."), {
        status: 409,
        code: "BUILDING_NUMBER_ALREADY_EXISTS",
      });
    throw cause;
  }
});
router.patch("/buildings/:buildingId", async (request, response) => {
  ...
  if (!keys.length || keys.some((key) => !allowed.includes(key)))
    throw Object.assign(new Error("Dati edificio non validi."), {
      status: 400,
      code: "VALIDATION_ERROR",
    });
  ...
  if (
    !Number.isInteger(Number(next.number)) ||
    ...
    Number(next.latitude) < -90 ||
    Number(next.latitude) > 90 ||
    !Number.isFinite(Number(next.longitude)) ||
    Number(next.longitude) < -180 ||
    Number(next.longitude) > 180
  )
    throw Object.assign(new Error("Dati edificio non validi."), {
      status: 400,
...
router.delete("/buildings/:buildingId", async (request, response) => {
  ...
  await transaction(async (db) => {
    await consolidateOccurrences(db);
    const building = await db.get("SELECT id FROM buildings WHERE id = ?;", [
      id,
    ]);
    if (!building)
      throw Object.assign(new Error("L’edificio richiesto non esiste."), {
    ...
    for (const space of await db.all(
      "SELECT id FROM spaces WHERE building_id=?",
      [id],
    ))
      await prepareSpaceDeletion(db, space.id);
...
router.get("/spaces", async (request, response) => {
  ...
  const search =
    typeof request.query.search === "string"
      ? request.query.search.trim().replace(/\s+/g, " ")
      : "";
  const values = where ? [`%${search}%`, `%${search}%`, search] : [];
  const total = await new Promise((resolve, reject) =>
    getDatabase().get(
      `SELECT COUNT(*) AS count FROM spaces sp JOIN buildings b ON b.id = sp.building_id ${where};`,
      values,
  ...
  const rows = await all(
    `SELECT sp.id, sp.name, sp.floor, sp.type, sp.capacity, sp.accessible, sp.status,
      ...
      ORDER BY sp.name, sp.id LIMIT ? OFFSET ?;`,
...
router.get("/spaces/:spaceId", async (request, response) => {
  ...
  const row = await new Promise((resolve, reject) =>
    getDatabase().get(
      `SELECT sp.id, sp.name, sp.floor, sp.type, sp.capacity, sp.accessible, sp.status,
            b.id AS buildingId, b.number AS buildingNumber, b.name AS buildingName
       FROM spaces sp JOIN buildings b ON b.id = sp.building_id WHERE sp.id = ?;`,
      [id],
      (error, value) => (error ? reject(error) : resolve(value)),
    ),
  );
  ...
  const serviceRows = await all(
    "SELECT code FROM services JOIN space_services ss ON ss.service_id=services.id WHERE ss.space_id=? ORDER BY code",
    [id],
  );
  response.json({
    data: {
      ...
      accessible: Boolean(row.accessible),
      status: row.status,
      serviceCodes: serviceRows.map((s) => s.code),
    },
  });
});
router.post("/spaces", async (request, response) => {
  ...
  const serviceCodes = Array.isArray(body.serviceCodes)
    ? [...new Set(body.serviceCodes)]
    : [];
  if (
    typeof body.name !== "string" ||
    !body.name.trim() ||
    !Number.isInteger(Number(body.buildingId)) ||
    Number(body.buildingId) < 1 ||
    !Number.isInteger(Number(body.floor)) ||
    !validTypes.includes(body.type) ||
    !Number.isInteger(Number(body.capacity)) ||
    Number(body.capacity) < 1 ||
    ![true, false, 0, 1].includes(body.accessible) ||
    ...
    throw Object.assign(new Error("I dati dello spazio non sono validi."), {
      status: 400,
      code: "VALIDATION_ERROR",
    });
  }
  const created = await transaction(async (db) => {
    ...
    const row = await db.run(
      ...
      [
        ...
        body.status === "active" && Boolean(body.accessible) ? 1 : 0,
        body.status,
      ],
    );
    for (const code of serviceCodes) {
      const service = await db.get("SELECT id FROM services WHERE code = ?;", [
        code,
      ]);
      if (service)
        await db.run(
          "INSERT INTO space_services (space_id, service_id) VALUES (?, ?);",
          [row.lastId, service.id],
        );
    }
    return row.lastId;
  });
...
router.patch("/spaces/:spaceId", async (request, response) => {
  ...
  const next = await transaction(async (db) => {
    ...
    const body = request.body || {};
    if (
      body.serviceCodes !== undefined &&
      (!Array.isArray(body.serviceCodes) ||
        body.serviceCodes.some(
          (code) =>
            ![
              "wifi",
              "power_outlets",
      ...
      throw Object.assign(new Error("Servizi non validi."), {
        status: 400,
        code: "VALIDATION_ERROR",
      });
    const next = { ...current, ...body };
    if (
      typeof next.name !== "string" ||
      !next.name.trim() ||
      !Number.isInteger(Number(body.buildingId ?? current.building_id)) ||
      !Number.isInteger(Number(next.floor)) ||
      !["study_room", "laboratory", "meeting_room"].includes(next.type) ||
      !Number.isInteger(Number(next.capacity)) ||
      Number(next.capacity) < 1 ||
      !["active", "maintenance", "deactivated"].includes(next.status)
    )
      throw Object.assign(new Error("Dati dello spazio non validi."), {
        status: 400,
        code: "VALIDATION_ERROR",
      });
    if (body.serviceCodes !== undefined) {
      await db.run("DELETE FROM space_services WHERE space_id=?", [id]);
      for (const code of new Set(body.serviceCodes))
        await db.run(
          "INSERT INTO space_services(space_id,service_id) SELECT ?,id FROM services WHERE code=?",
          [id, code],
        );
    }
    const capacity = Number(next.capacity);
    if (capacity < current.capacity) {
      const intervals = await db.all(
        `SELECT DISTINCT b.date,a.start_time AS startTime,a.end_time AS endTime
        FROM bookings b JOIN availabilities a ON a.id=b.availability_id
        WHERE b.space_id=? AND b.status='confirmed' AND b.date || ' ' || a.end_time > ?`,
        [id, romeDateTime()],
      );
      for (const interval of intervals) {
        if (
          ...
          throw Object.assign(
            new Error("La nuova capienza è inferiore ai posti già prenotati."),
            { status: 409, code: "CAPACITY_REDUCTION_CONFLICT" },
          );
      }
    }
    const statusChanged =
    ...
    if (statusChanged) {
      await rejectIfBookingInProgress(db, id);
      const participants = await db.all(
      ...
      for (const participant of participants)
        await db.run(
          `INSERT INTO notifications (user_id, type, title, message, created_at)
         VALUES (?, 'space_unavailable', 'Prenotazione cancellata', 'Una prenotazione è stata cancellata perché lo spazio non è disponibile.', ?);`,
          [participant.userId, createdAt],
        );
      await db.run(
        "DELETE FROM bookings WHERE space_id = ? AND status = 'confirmed' AND date || ' ' || (SELECT start_time FROM availabilities WHERE id=bookings.availability_id) > ?;",
        [id, romeDateTime()],
      );
    }
    await db.run(
      ...
      [
        Number(body.buildingId ?? current.building_id),
        next.name.trim(),
        Number(next.floor),
        next.type,
        capacity,
        next.status === "active" && Boolean(next.accessible) ? 1 : 0,
        next.status,
        id,
      ],
    );
    return {
      ...next,
      accessible: next.status === "active" && Boolean(next.accessible),
    };
  });
  response.json({
    data: {
...
router.get("/spaces/:spaceId/availability", async (request, response) => {
  ...
  const rows = await all(
    `SELECT id AS availabilityId, valid_from AS validFrom, valid_until AS validUntil, weekday,
       ...
       FROM availabilities WHERE space_id = ? AND superseded_by_id IS NULL ORDER BY valid_from, weekday, start_time, id;`,
    [spaceId],
  );
  response.json({
    data: rows.map((row) => ({ ...row, isRetired: Boolean(row.isRetired) })),
  });
});
router.post("/spaces/:spaceId/availability", async (request, response) => {
  ...
  if (
    ...
    spaceId < 1 ||
    !validDate(body.validFrom) ||
    !validDate(body.validUntil) ||
    body.validUntil < body.validFrom ||
    !Number.isInteger(Number(body.weekday)) ||
    Number(body.weekday) < 1 ||
    Number(body.weekday) > 7 ||
    !validTime(body.startTime) ||
    !validTime(body.endTime) ||
    body.endTime <= body.startTime
  )
    throw Object.assign(new Error("Dati disponibilità non validi."), {
      status: 400,
      code: "VALIDATION_ERROR",
    });
  const created = await transaction(async (db) => {
    await consolidateOccurrences(db);
    const space = await db.get("SELECT id FROM spaces WHERE id = ?;", [
      spaceId,
    ]);
    if (!space)
      throw Object.assign(new Error("Lo spazio richiesto non esiste."), {
    ...
    if (overlap)
      throw Object.assign(
        new Error("La fascia si sovrappone a una configurazione attiva."),
        { status: 409, code: "AVAILABILITY_OVERLAP" },
      );
...
router.post(
  "/spaces/:spaceId/availability/:availabilityId/reuse",
  async (request, response) => {
    ...
    const created = await transaction(async (db) => {
      ...
      const source = await db.get(
        "SELECT id FROM availabilities WHERE id=? AND space_id=? AND is_retired=1 AND superseded_by_id IS NULL",
        [availabilityId, spaceId],
      );
      if (!source)
        throw Object.assign(
          new Error("La fascia ritirata richiesta non esiste."),
          { status: 404, code: "AVAILABILITY_NOT_FOUND" },
        );
      const overlap = await db.get(
        `SELECT id FROM availabilities WHERE space_id=? AND is_retired=0 AND weekday=?
       AND valid_from<=? AND valid_until>=? AND start_time<? AND end_time>?`,
      ...
      await db.run("UPDATE availabilities SET superseded_by_id=? WHERE id=?", [
        replacement.lastId,
        availabilityId,
      ]);
  ...
  response.json({
    data: await all(
      `SELECT id, date, start_time AS startTime, end_time AS endTime, reason
       FROM unavailabilities WHERE space_id = ? ORDER BY date, start_time, id;`,
      [spaceId],
    ),
  });
});
async function saveUnavailability(request, response) {
  ...
  const unavailabilityId =
    request.params.unavailabilityId === undefined
      ? null
      : Number(request.params.unavailabilityId);
  if (
    unavailabilityId !== null &&
    (!Number.isInteger(unavailabilityId) || unavailabilityId < 1)
  )
    throw Object.assign(
      new Error("Identificativo indisponibilità non valido."),
      { status: 400, code: "VALIDATION_ERROR" },
    );
  if (
    !Number.isInteger(spaceId) ||
    ...
    throw Object.assign(new Error("Dati indisponibilità non validi."), {
      status: 400,
      code: "VALIDATION_ERROR",
    });
  const id = await transaction(async (db) => {
    ...
    if (
      unavailabilityId !== null &&
      !(await db.get(
        "SELECT id FROM unavailabilities WHERE id=? AND space_id=?",
        [unavailabilityId, spaceId],
      ))
    )
      throw Object.assign(
        new Error("L’indisponibilità richiesta non esiste."),
        { status: 404, code: "UNAVAILABILITY_NOT_FOUND" },
      );
    const now = new Date();
    await rejectIfBookingInProgress(db, spaceId, now, body);
    ...
    await cancelBookings(
      ...
      conflicts,
      now.toISOString(),
      "La prenotazione è stata cancellata per indisponibilità dello spazio.",
    );
    if (unavailabilityId !== null) {
      await db.run(
        "UPDATE unavailabilities SET date=?,start_time=?,end_time=?,reason=? WHERE id=? AND space_id=?",
        [
          body.date,
          body.startTime,
          body.endTime,
  ...
  response
    .status(unavailabilityId === null ? 201 : 200)
    .json({
      data: {
        id,
        spaceId,
...
router.patch(
  "/spaces/:spaceId/unavailability/:unavailabilityId",
  saveUnavailability,
);
router.patch(
  "/spaces/:spaceId/availability/:availabilityId",
  async (request, response) => {
    const spaceId = Number(request.params.spaceId);
    const availabilityId = Number(request.params.availabilityId);
    const body = request.body || {};
    ...
    const created = await transaction(async (db) => {
      ...
      const current = await db.get(
        "SELECT id FROM availabilities WHERE id = ? AND space_id = ? AND is_retired = 0;",
        [availabilityId, spaceId],
      );
      if (!current)
        throw Object.assign(
          new Error("La disponibilità richiesta non esiste."),
          { status: 404, code: "AVAILABILITY_NOT_FOUND" },
        );
      const overlap = await db.get(
      ...
      await db.run("UPDATE availabilities SET is_retired = 1 WHERE id = ?;", [
        availabilityId,
      ]);
      const replacement = await db.run(
        `INSERT INTO availabilities (space_id, valid_from, valid_until, weekday, start_time, end_time, is_retired)
...
router.delete(
  ...
  async (request, response) => {
    ...
    await transaction(async (db) => {
      await consolidateOccurrences(db);
      const result = await db.run(
        "UPDATE availabilities SET is_retired = 1 WHERE id = ? AND space_id = ? AND is_retired = 0;",
        [availabilityId, spaceId],
      );
      if (!result.changes)
        throw Object.assign(
...
router.delete(
  ...
  async (request, response) => {
    const spaceId = Number(request.params.spaceId);
    const unavailabilityId = Number(request.params.unavailabilityId);
    await transaction(async (db) => {
      await consolidateOccurrences(db);
      const result = await db.run(
        "DELETE FROM unavailabilities WHERE id=? AND space_id=?",
        [unavailabilityId, spaceId],
      );
      if (!result.changes)
        throw Object.assign(
...
router.get("/bookings", async (request, response) => {
  ...
  const rows = await all(
    `SELECT b.id, b.date, b.status, b.created_at AS createdAt, s.id AS spaceId, s.name AS spaceName,
      ...
      GROUP BY b.id ORDER BY b.date DESC, b.id DESC LIMIT ? OFFSET ?;`,
    [size, (page - 1) * size],
  );
  ...
  for (const row of rows) {
    ...
    row.participants = await all(
      ...
      FROM booking_participants bp JOIN users u ON u.id=bp.user_id WHERE bp.booking_id=? ORDER BY bp.id`,
      [row.id],
    );
    row.participants = row.participants.map((person) => ({
      ...person,
...
router.get("/reports", async (request, response) => {
  const rows =
    await all(`SELECT r.id, r.space_id AS spaceId, s.name AS spaceName, r.category, r.description,
...
router.get("/reports/:reportId/photo", async (request, response) => {
  ...
  if (!Number.isSafeInteger(id) || id < 1)
    throw Object.assign(new Error("Identificativo segnalazione non valido."), {
      status: 400,
    });
  const report = await get(
    "SELECT photo_path AS photo, photo_data AS photoData, photo_type AS photoType FROM reports WHERE id = ?;",
    [id],
  );
  if (!report || (!report.photo && !report.photoData))
    throw Object.assign(new Error("Foto non disponibile."), {
      status: 404,
      code: "PHOTO_NOT_FOUND",
    });
  await sendReportPhoto(
    report.photo,
    response,
    report.photoData,
    report.photoType,
...
router.patch("/reports/:reportId/status", async (request, response) => {
  ...
  if (
    ...
    throw Object.assign(new Error("Stato segnalazione non valido."), {
      status: 400,
  ...
  await transaction(async (db) => {
    ...
    const report = await db.get(
      "SELECT id, user_id AS userId, status FROM reports WHERE id = ?;",
      [id],
    );
    if (!report)
      throw Object.assign(new Error("La segnalazione richiesta non esiste."), {
        status: 404,
    ...
    if (
      (report.status === "open" &&
        !["in_progress", "resolved"].includes(nextStatus)) ||
      (report.status === "in_progress" && nextStatus !== "resolved") ||
      report.status === "resolved"
    )
      throw Object.assign(new Error("Transizione di stato non consentita."), {
        status: 409,
    ...
    await db.run(
      ...
      [
        report.userId,
        `La tua segnalazione è ora ${nextStatus === "in_progress" ? "in lavorazione" : "risolta"}.`,
        updatedAt,
      ],
    );
  });
...
router.get("/users", async (request, response) => {
  ...
  const where = search
    ? "AND (u.first_name LIKE ? OR u.last_name LIKE ? OR u.email LIKE ? OR TRIM(u.first_name) || ' ' || TRIM(u.last_name) LIKE ?)"
    : "";
  const values = search
    ? [`%${search}%`, `%${search}%`, `%${search}%`, `%${search}%`]
    : [];
  const total = await getDatabase().get(
    `SELECT COUNT(*) AS total FROM users u WHERE 1 = 1 ${where};`,
    values,
  );
  const rows = await all(
    `SELECT u.id, u.first_name AS firstName, u.last_name AS lastName, u.email, u.role, u.created_at AS createdAt
       FROM users u WHERE 1 = 1 ${where} ORDER BY u.last_name, u.first_name, u.id LIMIT ? OFFSET ?;`,
    [...values, size, (page - 1) * size],
  );
  response.json({
    data: rows,
...
router.get("/users/:userId", async (request, response) => {
  ...
  const row = await new Promise((resolve, reject) =>
    getDatabase().get(
      "SELECT id, first_name AS firstName, last_name AS lastName, email, role, created_at AS createdAt FROM users WHERE id = ?;",
      [id],
      (error, value) => (error ? reject(error) : resolve(value)),
    ),
  );
  if (!row)
    throw Object.assign(new Error("L’utente richiesto non esiste."), {
      status: 404,
      code: "USER_NOT_FOUND",
    });
  response.json({ data: row });
...
router.delete("/users/:userId", async (request, response) => {
  const id = Number(request.params.userId);
  if (!Number.isInteger(id) || id < 1)
    throw Object.assign(new Error("Identificativo utente non valido."), {
      status: 400,
  ...
  await transaction(async (db) => {
    ...
    if (user.role === "admin")
      throw Object.assign(
        new Error(
          "Gli amministratori non possono essere eliminati da questa funzione.",
        ),
    ...
    const participants = await db.all(
      `SELECT DISTINCT bp.user_id AS userId FROM booking_participants bp JOIN bookings b ON b.id=bp.booking_id
       ...
       AND bp.user_id<>? AND b.status='confirmed' AND b.date || ' ' || a.start_time > ?`,
      [id, id, romeDateTime()],
    );
    await queuePhotos(
      db,
    ...
    for (const participant of participants)
      await db.run(
        `INSERT INTO notifications (user_id, type, title, message, created_at)
       VALUES (?, 'booking_cancelled', 'Prenotazione cancellata', 'Una prenotazione è stata cancellata perché l’account è stato eliminato.', ?);`,
        [participant.userId, createdAt],
      );
    await db.run(
      "DELETE FROM bookings WHERE id IN (SELECT booking_id FROM booking_participants WHERE user_id = ? AND participant_role = 'organizer');",
      [id],
    );
