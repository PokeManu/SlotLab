...
const { checkInWindow, isMoreThanAhead, romeNow } = require("../domain/time");
...
async function qualifyingSpaces({
  availableNow = false,
  minSeats,
  todayOnly = false,
  tomorrowOnly = false,
} = {}) {
  const dates = tomorrowOnly
    ? dateRange(1).slice(1)
    : dateRange(todayOnly ? 0 : 30);
  const nowInstant = new Date();
  const now = romeNow(nowInstant);
  const spaces = await query(
    "SELECT id, capacity, status FROM spaces WHERE status = 'active';",
  );
  const result = [];
  for (const space of spaces) {
    const availabilities = await query(
      `SELECT id, valid_from AS validFrom, valid_until AS validUntil, weekday, start_time AS startTime, end_time AS endTime
         FROM availabilities WHERE space_id = ? AND is_retired = 0 ORDER BY start_time, id;`,
      [space.id],
    );
    let match = null;
    for (const date of dates) {
      for (const slot of availabilities) {
        if (
          ...
          date.value < slot.validFrom ||
          date.value > slot.validUntil
        )
          continue;
        try {
          if (!isMoreThanAhead(date.value, slot.startTime, 60 * 60, nowInstant))
            continue;
        } catch (cause) {
...
async function servicesFor(spaceIds) {
  ...
  const rows = await query(
    `SELECT ss.space_id AS spaceId, s.code
      ...
      WHERE ss.space_id IN (${placeholders}) ORDER BY s.code;`,
    spaceIds,
  );
...
router.get("/buildings", async (request, response) => {
  const rows =
    await query(`SELECT id, number, name, address, latitude, longitude
...
async function spacesResponse(request, response, buildingId = null) {
  ...
  const minSeats =
    request.query.minSeats === undefined
      ? undefined
      : Number(request.query.minSeats);
  if (
    request.query.availableNow !== undefined &&
    request.query.availableNow !== "true" &&
    request.query.availableNow !== "false"
  )
    throw invalidPagination();
  if (
    minSeats !== undefined &&
    (!Number.isInteger(minSeats) || minSeats < 1 || minSeats > 1000)
  )
    throw invalidPagination();
  const conditions = [];
  const parameters = [];
  if (buildingId !== null) {
    conditions.push("sp.building_id = ?");
    parameters.push(buildingId);
  }
  if (accessible !== undefined) {
    conditions.push("sp.accessible = ?");
    parameters.push(accessible);
  }
  if (type !== undefined) {
    if (!["study_room", "laboratory", "meeting_room"].includes(type))
      throw invalidPagination();
  ...
  if (search) {
    conditions.push(
      "(LOWER(sp.name) LIKE LOWER(?) OR LOWER(b.name) LIKE LOWER(?) OR CAST(b.number AS TEXT) = ?)",
    );
  ...
  const count = await get(
    `SELECT COUNT(*) AS total FROM spaces sp JOIN buildings b ON b.id = sp.building_id ${where};`,
    parameters,
  );
  const rows = await query(
    `SELECT sp.id, sp.name, sp.floor, sp.type, sp.capacity, sp.accessible, sp.status,
      ...
      ORDER BY sp.name ASC, sp.id ASC LIMIT ? OFFSET ?;`,
...
router.post(
  "/spaces/:spaceId/check-in",
  rateLimit({ limit: 30, windowMs: 60000, key: (request) => request.user.id }),
  async (request, response) => {
    ...
    const result = await transaction(async (db) => {
      ...
      const bookings = await db.all(
        ...
        ORDER BY b.date ASC, a.start_time ASC;`,
        [spaceId, request.user.id],
      );
      if (!bookings.length)
        throw Object.assign(
          new Error("Non esiste una prenotazione per questo spazio."),
          {
            status: 404,
            code: "NO_BOOKING_FOR_SPACE",
          },
        );
      const now = new Date();
      ...
      const booking =
        bookings.find(
          (item) => checkInStatus(item.date, item.startTime) === null,
        ) ??
        bookings.find(
          (item) => checkInStatus(item.date, item.startTime) === "early",
        ) ??
        bookings[bookings.length - 1];
      if (booking.present) return { booking, repeated: true };
      const windowError = checkInStatus(booking.date, booking.startTime);
      if (windowError === "early")
        throw Object.assign(new Error("Il check-in è troppo anticipato."), {
          status: 409,
          code: "CHECK_IN_TOO_EARLY",
        });
      if (windowError === "expired")
        throw Object.assign(new Error("Il check-in è scaduto."), {
          status: 409,
      ...
      await db.run(
        ...
        WHERE booking_id = ? AND user_id = ? AND present = 0;`,
        [checkedInAt, booking.bookingId, request.user.id],
      );
    ...
    if (result.repeated)
      return response.json({
        data: {
          ...
          message: "Accesso già registrato.",
...
router.get("/spaces/:spaceId/availability", async (request, response) => {
  ...
  const space = await get("SELECT capacity, status FROM spaces WHERE id = ?;", [
    spaceId,
  ]);
  if (!space)
    throw Object.assign(new Error("Lo spazio richiesto non esiste."), {
  ...
  const data = await Promise.all(
    slots.map(async (slot) => {
      ...
      const bookedSeats = await occupiedSeats(
        { all: query },
        spaceId,
        requestedDate.value,
        slot.startTime,
...
router.get("/spaces", async (request, response) =>
  spacesResponse(request, response),
);
router.get("/spaces/recommended", async (request, response) => {
  ...
  const selected = recommendations.length
    ? recommendations
    : await qualifyingSpaces({ tomorrowOnly: true });
  const rows = await query(
    `SELECT sp.id, sp.name, sp.floor, sp.type, sp.capacity, sp.accessible, sp.status,
            b.id AS buildingId, b.number AS buildingNumber, b.name AS buildingName
       FROM spaces sp JOIN buildings b ON b.id = sp.building_id WHERE sp.id IN (${
         selected
           .slice(0, 3)
           .map(() => "?")
           .join(",") || "NULL"
