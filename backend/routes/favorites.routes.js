...
function spaceId(value) {
  if (!/^[1-9][0-9]*$/.test(String(value))) {
    throw Object.assign(new Error('Identificativo spazio non valido.'), { status: 400, code: 'INVALID_SPACE_ID' });
  }
  return Number(value);
}
...
router.get('/', async (request, response) => {
  ...
  const total = await new Promise((resolve, reject) => getDatabase().get(
    'SELECT COUNT(*) AS count FROM favorites WHERE user_id = ?;', parameters,
  ...
  response.json({ data: rows.map(row => ({
    id: row.id, name: row.name, building: { id: row.buildingId, number: row.buildingNumber, name: row.buildingName },
    floor: row.floor, type: row.type, capacity: row.capacity, accessible: Boolean(row.accessible), status: row.status,
    imageType: row.type, isFavorite: true,
...
router.post('/:spaceId', async (request, response) => {
  ...
  const exists = await new Promise((resolve, reject) => getDatabase().get('SELECT id FROM spaces WHERE id = ?;', [id], (error, row) => error ? reject(error) : resolve(row)));
  if (!exists) throw Object.assign(new Error('Lo spazio richiesto non esiste.'), { status: 404, code: 'SPACE_NOT_FOUND' });
  ...
  response.status(result.changes === 1 ? 201 : 200).json({ data: { spaceId: id, isFavorite: true } });
});

router.delete('/:spaceId', async (request, response) => {
  const id = spaceId(request.params.spaceId);
  await run('DELETE FROM favorites WHERE user_id = ? AND space_id = ?;', [request.user.id, id]);
