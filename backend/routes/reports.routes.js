...
function mapReport(row) {
  return {
    ...
    status: row.status,
    photo: row.hasPhoto ? "available" : null,
...
function parseMultipart(request) {
  return new Promise((resolve, reject) => {
    const match =
      /^multipart\/form-data;\s*boundary=(?:"([^"]+)"|([^;]+))$/i.exec(
        request.get("Content-Type") || "",
      );
    if (!match)
      return reject(
        Object.assign(
          new Error("La segnalazione richiede multipart/form-data."),
    ...
    request.on("data", (chunk) => {
      ...
      if (total > 5 * 1024 * 1024 + 65536) {
        ...
        reject(
          Object.assign(
            new Error("La richiesta supera il limite consentito."),
            { status: 413, code: "PHOTO_TOO_LARGE" },
          ),
        );
        return;
      }
    ...
    request.on("end", () => {
      ...
      for (const part of body
        .toString("latin1")
        .split(boundary.toString("latin1"))
        .slice(1)) {
        ...
        const disposition = /name="([^"]+)"(?:;\s*filename="([^"]*)")?/i.exec(
          headers,
        );
...
function photoFormat(photo) {
  ...
  if (photo.data.length > 5 * 1024 * 1024)
    throw Object.assign(new Error("La foto supera 5 MB."), {
      status: 413,
      code: "PHOTO_TOO_LARGE",
    });
  if (photo.data.subarray(0, 3).equals(Buffer.from([0xff, 0xd8, 0xff])))
    return "jpg";
  if (
    photo.data
      .subarray(0, 8)
    ...
    return "png";
  if (
    photo.data.subarray(0, 4).toString() === "RIFF" &&
    photo.data.subarray(8, 12).toString() === "WEBP"
  )
    return "webp";
  throw Object.assign(new Error("Il formato della foto non è valido."), {
    status: 415,
...
router.get("/", async (request, response) => {
  ...
  const rows = await query(
    `${select} WHERE r.user_id = ? ORDER BY r.created_at DESC, r.id DESC LIMIT ? OFFSET ?;`,
...
router.get("/:reportId/photo", async (request, response) => {
  ...
  const report = await get(
    "SELECT photo_path AS photo, photo_data AS photoData, photo_type AS photoType FROM reports WHERE id = ? AND user_id = ?;",
    [id, request.user.id],
  );
...
router.get("/:reportId", async (request, response) => {
  const id = reportId(request.params.reportId);
  const row = await get(`${select} WHERE r.id = ? AND r.user_id = ?;`, [
    id,
    request.user.id,
  ]);
  if (!row)
...
spaceReportsRouter.post(
  "/spaces/:spaceId/reports",
  async (request, response) => {
    if (!/^[1-9][0-9]*$/.test(request.params.spaceId))
    ...
    if (!Object.hasOwn(priorities, category))
      throw Object.assign(new Error("La categoria non è valida."), {
        status: 400,
        code: "INVALID_REPORT_CATEGORY",
      });
    if (!description)
      throw Object.assign(new Error("La descrizione è obbligatoria."), {
        status: 400,
