...
function unauthorized() {
  return Object.assign(
    new Error("Autenticazione richiesta o sessione non valida."),
    {
      status: 401,
      code: "UNAUTHORIZED",
    },
...
async function requireAuth(request, response, next) {
  ...
  const match =
    typeof authorization === "string" && /^Bearer +(\S+)$/i.exec(authorization);
  ...
  if (
    ...
    claims.exp - claims.iat !== auth.accessTokenSeconds ||
...
function requireRole(...roles) {
  return (request, response, next) => {
    ...
    if (!roles.includes(request.user.role)) {
      return next(
        Object.assign(new Error("Non hai i permessi per questa operazione."), {
          status: 403,
          code: "FORBIDDEN",
        }),
      );
