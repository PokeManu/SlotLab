...
function validationError(
  message = "Il periodo delle statistiche non è valido.",
) {
...
function romeParts(value) {
  ...
  return {
    ...
    month: parts.date.slice(5, 7),
    day: parts.date.slice(8, 10),
    hour: parts.time.slice(0, 2),
    minute: parts.time.slice(3, 5),
  };
}
...
function noSuchTable(error, tableName) {
  return (
    error &&
    new RegExp(`no such table: ${tableName}`, "i").test(
...
async function readTracking(db) {
  try {
    return await db.get(`SELECT tracking_started_at AS trackingStartedAt,
...
function historyCompleteness(
  ...
  const consolidatedLocal = consolidated
    ? romeDateTime(consolidated).local
    : null;
  const measuredEnd =
    dateTo < nowLocal.slice(0, 10) ? `${dateTo} 23:59` : nowLocal;
  if (!consolidatedLocal || consolidatedLocal < measuredEnd) return false;
  if (issues.length) return false;
  return !endedBookings.some(
    (booking) =>
      !hasFinalizedSnapshot(occurrencesByKey.get(bookingKey(booking))),
  );
}
async function statistics(db, dateFrom, dateTo, now = new Date()) {
  const period = validateStatisticsPeriod({ dateFrom, dateTo });
  if (!(now instanceof Date) || Number.isNaN(now.getTime()))
    throw new TypeError("now deve essere una data valida.");
  ...
  const reportRows = await db.all(
    ...
    [
      ...
      new Date(
        localDateTimeToInstant(period.dateTo, "23:59:59").getTime() + 1000,
      ).toISOString(),
    ],
  );
  const tracking = await readTracking(db);
  const occurrenceIssues = await readOccurrenceIssues(
    db,
    period.dateFrom,
    period.dateTo,
  );
  const terminatedOccurrences = occurrenceRows.filter((row) =>
    isTerminatedOccurrence(row, now),
  );
  const offeredOccurrences = terminatedOccurrences.filter(
    (row) => hasFinalizedSnapshot(row) && Number(row.wasOffered) === 1,
  );
  const offeredByKey = new Map(
    offeredOccurrences.map((row) => [occurrenceKey(row), row]),
  );
  const occurrencesByKey = new Map(
    terminatedOccurrences.map((row) => [occurrenceKey(row), row]),
  );
  const endedBookings = bookingRows.filter((row) =>
    isTerminatedOccurrence(row, now),
  );
  const cohortBookings = endedBookings.filter((row) =>
    offeredByKey.has(bookingKey(row)),
  );
  const participantByBooking = new Map();
  for (const row of participantRows) {
    if (!participantByBooking.has(row.bookingId))
      participantByBooking.set(row.bookingId, []);
    participantByBooking.get(row.bookingId).push(row);
  }
  const bookingIds = new Set(cohortBookings.map((row) => row.bookingId));
  const participantCount = cohortBookings.reduce(
    (total, booking) =>
      total + (participantByBooking.get(booking.bookingId)?.length || 0),
    0,
  );
  const presenceCount = cohortBookings.reduce(
    (total, booking) =>
      total +
      (participantByBooking.get(booking.bookingId) || []).filter((row) =>
        isPresent(row.present),
      ).length,
    0,
  );
  const offeredCapacity = offeredOccurrences.reduce(
    (total, occurrence) => total + Number(occurrence.offeredCapacity || 0),
    0,
  );
  const dailyMap = new Map();
  for (const occurrence of offeredOccurrences) {
    if (!dailyMap.has(occurrence.date))
      dailyMap.set(occurrence.date, new Set());
  }
  for (const booking of cohortBookings)
    dailyMap.get(booking.date)?.add(booking.bookingId);
  const daily = [...dailyMap.entries()]
    .sort(([first], [second]) => first.localeCompare(second))
    .map(([date, ids]) => ({ date, bookings: ids.size }));
  const usageMap = new Map();
  for (const occurrence of offeredOccurrences) {
    if (!usageMap.has(occurrence.spaceType))
      usageMap.set(occurrence.spaceType, 0);
  }
  ...
  for (const booking of cohortBookings) {
    ...
    usageMap.set(
      occurrence.spaceType,
      (usageMap.get(occurrence.spaceType) || 0) + presences,
    );
  }
  const usageTotal = [...usageMap.values()].reduce(
  ...
  const usage = [...usageMap.entries()]
    ...
    .map(([type, presences]) => ({
      type,
      presences,
      percentage: roundRatio(presences, usageTotal),
    }));
  const mostBookedSpaces = [...spacesMap.values()].sort(
    (first, second) =>
      second.bookings - first.bookings ||
      second.participants - first.participants ||
      first.spaceName.localeCompare(second.spaceName) ||
      first.spaceId - second.spaceId,
  );
  const mostUsedSlots = [...slotsMap.values()].sort(
    (first, second) =>
      second.bookings - first.bookings ||
      second.presences - first.presences ||
      second.participants - first.participants ||
  ...
  for (const report of reportRows) {
    ...
    if (
      !reportDate ||
      reportDate < period.dateFrom ||
      reportDate > period.dateTo
    )
      continue;
    reportsByCategoryMap.set(
      report.category,
      (reportsByCategoryMap.get(report.category) || 0) + 1,
    );
    reportsByStatusMap.set(
      report.status,
      (reportsByStatusMap.get(report.status) || 0) + 1,
    );
  }
  const reportsByCategory = [...reportsByCategoryMap.entries()]
    .sort(([first], [second]) => first.localeCompare(second))
    .map(([category, count]) => ({ category, count }));
  const reportsByStatus = [...reportsByStatusMap.entries()]
    .sort(([first], [second]) => first.localeCompare(second))
    .map(([status, count]) => ({ status, count }));
  const ambiguousIssueKeys = new Set(
    occurrenceIssues
      .filter(
        (issue) =>
          issue.issue === "ambiguous_time" ||
          issue.issue === "overlapping_intervals",
      )
      .map((issue) => occurrenceKey(issue)),
  );
  const gapIssueKeys = new Set(
    occurrenceIssues
  ...
  const hasAmbiguousOccurrences =
    offeredOccurrences.some((first, index) =>
      offeredOccurrences
        .slice(index + 1)
        .some(
          (second) =>
    ...
    ambiguousIssueKeys.size > 0 ||
    gapIssueKeys.size > 0;
  const history = {
    trackingStartedAt: tracking?.trackingStartedAt || null,
    complete:
      !hasAmbiguousOccurrences &&
      terminatedOccurrences.every(hasFinalizedSnapshot) &&
      historyCompleteness(
        tracking,
        period.dateFrom,
        period.dateTo,
        romeDateTime(now).local,
        endedBookings,
        occurrencesByKey,
        occurrenceIssues,
      ),
    excludedBookings: endedBookings.filter(
      (booking) => !offeredByKey.has(bookingKey(booking)),
    ).length,
  ...
  return {
    ...
    offeredCapacity,
    utilizationRate: hasAmbiguousOccurrences
      ? null
      : roundRatio(presenceCount, offeredCapacity),
