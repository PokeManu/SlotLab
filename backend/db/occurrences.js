...
function occurrenceInstant(date, time) {
  try {
    ...
    const later = localDateTimeToInstant(date, time, {
    ...
    if (error?.code === "INVALID_LOCAL_TIME")
      return { instant: null, issue: "nonexistent_time" };
    throw error;
  }
}
function occurrenceInterval(item) {
  ...
  if (!start.instant || !end.instant || end.instant <= start.instant) {
    return {
      ...
      instant: null,
      issue: issues[0] || "nonexistent_time",
    };
  }
...
async function ensureTracking(db, nowIso) {
  const current = await db.get(
    "SELECT id, tracking_started_at AS trackingStartedAt, consolidated_until AS consolidatedUntil FROM occurrence_tracking WHERE id = 1;",
  );
...
function inWindow(item, cursor, now) {
  ...
  if (interval.instant)
    return interval.instant > cursor && interval.instant <= now;
  const localStart = `${item.date} ${item.startTime}`;
  return (
    localStart > localDateTimeString(new Date(cursor)) &&
    localStart <= localDateTimeString(new Date(now))
  );
}
async function availabilityCandidates(db, cursor, now, firstDate, lastDate) {
  ...
  for (const availability of rows) {
    let date =
      availability.validFrom > firstDate ? availability.validFrom : firstDate;
    const endDate =
      availability.validUntil < lastDate ? availability.validUntil : lastDate;
    while (date <= endDate) {
      if (weekday(date) === availability.weekday) {
        const item = {
          ...
          isRetired: Boolean(availability.isRetired),
        };
  ...
  for (const group of grouped.values()) {
    for (let leftIndex = 0; leftIndex < group.length; leftIndex += 1) {
      for (
        ...
        if (!intervalOverlaps(left, right) || keyFor(left) === keyFor(right))
          continue;
        await recordIssue(
          db,
          left,
          "overlapping_intervals",
          `Sovrapposizione con ${right.startTime}-${right.endTime}.`,
          detectedAt,
        );
        await recordIssue(
          db,
          right,
          "overlapping_intervals",
          `Sovrapposizione con ${left.startTime}-${left.endTime}.`,
          detectedAt,
        );
...
async function consolidateOccurrences(db, now = new Date()) {
  if (!(now instanceof Date) || Number.isNaN(now.getTime()))
    throw new TypeError("Ora di consolidamento non valida.");
  const nowIso = now.toISOString();
  const tracking = await ensureTracking(db, nowIso);
  const cursor = new Date(tracking.current.consolidatedUntil);
  let completedBookings = 0;
  const endedCandidates = await db.all(
    `SELECT b.id,b.date,a.end_time AS endTime
    FROM bookings b JOIN availabilities a ON a.id=b.availability_id
  ...
  const candidates = await availabilityCandidates(
    db,
    cursor,
    now.getTime(),
    firstDate,
  ...
  for (const item of candidates) {
    ...
    if (!interval.instant) {
      await recordIssue(
        ...
        item,
        "nonexistent_time",
        "Orario locale inesistente nel fuso Europe/Rome.",
        detectedAt,
      );
      gapOccurrences += 1;
      continue;
    ...
    if (interval.issue) {
      await recordIssue(
        ...
        item,
        interval.issue,
        "Orario locale ambiguo nel fuso Europe/Rome; usato il primo istante.",
        detectedAt,
      );
    ...
    const space = await db.get(
      "SELECT capacity, status FROM spaces WHERE id = ?;",
      [item.spaceId],
    );
    if (!space) continue;
  ...
  const existingRows = await db.all(
    `SELECT space_id AS spaceId, date, start_time AS startTime, end_time AS endTime
      ...
      WHERE date BETWEEN ? AND ? AND finalized_at IS NOT NULL AND was_offered = 1;`,
    [firstDate, lastDate],
  );
  const overlapCount = await markOverlaps(db, existingRows, detectedAt);
  await db.run(
    "UPDATE occurrence_tracking SET consolidated_until = ? WHERE id = 1;",
    [nowIso],
  );
  return {
