...
const DEMO_REPORTS = [
  {
    ...
    priority: "high",
    status: "open",
    description: "Presa elettrica non funzionante.",
  },
  {
    category: "technical",
    priority: "high",
    status: "in_progress",
    description: "Proiettore con immagine intermittente.",
  },
  {
    category: "accessibility",
    priority: "high",
    status: "resolved",
    description: "Accesso alla postazione ostacolato.",
  },
  {
    category: "cleaning",
    priority: "medium",
    status: "open",
    description: "Tavoli da pulire.",
  },
  {
    category: "cleaning",
    priority: "medium",
  ...
  },
  {
    ...
    priority: "low",
    status: "in_progress",
    description: "Rumore eccessivo nella sala.",
  },
  {
    category: "other",
    priority: "low",
...
function selectedDatabasePath(environment = process.env) {
  if (!environment.SLOTLAB_DEMO_DB_PATH) {
    throw new Error(
      "SLOTLAB_DEMO_DB_PATH deve indicare un database separato di prova.",
    );
  }
  const selected = path.resolve(environment.SLOTLAB_DEMO_DB_PATH);
  const operational = path.resolve(
    environment.SLOTLAB_DB_PATH || path.join(__dirname, "database.sqlite"),
  );
...
async function ensureDemoUsers(db) {
  ...
  for (const [firstName, lastName, email] of DEMO_USERS) {
    let user = await db.get("SELECT id, email FROM users WHERE email = ?;", [
      email,
    ]);
    if (!user) {
...
async function replaceActiveAvailabilities(db, spaces) {
  const desiredTimes = new Set(
    SLOT_TIMES.map((slot) => `${slot.startTime}-${slot.endTime}`),
  );
  for (const space of spaces) {
    ...
    for (const row of activeRows) {
      const isDemoSlot =
        row.validFrom === PERIOD.validFrom &&
...
function checkedInAt(date, startTime, minutesBefore) {
  ...
  return new Date(start.getTime() - minutesBefore * 60000).toISOString();
}
  ...
  for (let memberIndex = 0; memberIndex < groupSize; memberIndex += 1) {
    ...
    await db.run(
      ...
      [
        bookingId,
        user.id,
        memberIndex === 0 ? "organizer" : "participant",
        present ? 1 : 0,
        present ? checkedInAt(date, startTime, 5 + memberIndex) : null,
      ],
    );
  }
}
async function createDemoBookings(db, spaces, users, today) {
  ...
  for (const [spaceIndex, space] of spaces.entries()) {
    ...
    for (const [exampleIndex, example] of examples.entries()) {
      ...
      const organizerIndex =
        (spaceIndex * examples.length + exampleIndex) % users.length;
  ...
  return {
    ...
    presences,
    absences: completedParticipants - presences,
  };
}
async function createDemoReports(db, spaces, users, today) {
  ...
  for (const [index, report] of DEMO_REPORTS.entries()) {
    ...
    const updatedAt =
      report.status === "open"
        ? createdAt
        : new Date(new Date(createdAt).getTime() + 60 * 60000).toISOString();
    await db.run(
      `INSERT INTO reports
      ...
      [
        ...
        spaces[index % spaces.length].id,
        report.category,
        `[Demo statistiche] ${report.description}`,
        report.priority,
        report.status,
        createdAt,
        updatedAt,
...
async function seedDemo(options = {}) {
  ...
  const databasePath =
    options.databasePath || selectedDatabasePath(environment);
  ...
  try {
    ...
    if (!spaces.length)
      throw new Error(
        "Nessuno spazio presente: eseguire prima npm run db:seed.",
      );
    const users = await ensureDemoUsers(db);
    await removePreviousDemoBookings(
      db,
      users.map((user) => user.id),
    );
    await replaceActiveAvailabilities(db, spaces);
    const bookingSummary = await createDemoBookings(
      db,
      spaces,
      users,
      romeNow().date,
    );
    const reportCount = await createDemoReports(
      db,
      spaces,
      users,
      romeNow().date,
    ...
    return {
      ...
      bookingCount: bookingSummary.total,
      completedBookingCount: bookingSummary.completed,
      futureBookingCount: bookingSummary.future,
      participantCount: bookingSummary.participants,
      presenceCount: bookingSummary.presences,
      absenceCount: bookingSummary.absences,
      reportCount,
...
if (require.main === module) {
  seedDemo()
    .then((result) => {
      ...
      console.log(
        `Prenotazioni demo inserite: ${result.bookingCount} (${result.completedBookingCount} concluse, ${result.futureBookingCount} future)`,
      );
      console.log(
        `Partecipazioni concluse: ${result.participantCount} (${result.presenceCount} presenze, ${result.absenceCount} assenze)`,
      );
