...
function parseDateTimeParts(date, time) {
  ...
  if (!dateMatch || !timeMatch) {
    throw Object.assign(new RangeError('Data o orario Europe/Rome non valido.'), {
...
function sameLocalParts(actual, expected) {
  return actual.year === String(expected.year).padStart(4, '0') &&
...
/**
 * Converte una data/ora del calendario Europe/Rome in un istante UTC.
 *
 * Durante il ritorno all'ora solare un orario può esistere due volte: per
 * convenzione scegliamo la prima occorrenza ("earlier"). Il salto primaverile
 * produce invece zero occorrenze e viene rifiutato, evitando slot inesistenti.
 */
function localDateTimeToInstant(date, time, options = {}) {
  const expected = parseDateTimeParts(date, time);
  const wallClock = Date.UTC(
    expected.year, expected.month - 1, expected.day,
  ...
  if (candidates.length === 0) {
    throw Object.assign(new RangeError('L’orario locale non esiste in Europe/Rome.'), {
      code: 'INVALID_LOCAL_TIME',
    });
  }
  if (candidates.length > 1 && options.disambiguation === 'reject') {
    throw Object.assign(new RangeError('L’orario locale è ambiguo in Europe/Rome.'), {
...
function secondsUntilLocal(date, time, instant = new Date()) {
  return (localDateTimeToInstant(date, time).getTime() - instant.getTime()) / 1000;
}
