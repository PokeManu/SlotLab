...
function cleanDeletedFiles() {
  ...
  cleaning = (async () => {
    ...
    const root =
      process.env.SLOTLAB_UPLOAD_DIR ||
      path.join(__dirname, "..", "uploads", "reports");
    if (!path.isAbsolute(root))
      throw new Error("La cartella foto deve essere assoluta.");
    const files = await query.all("SELECT filename FROM file_deletions");
    for (const file of files) {
      try {
        const filename = photoName(file.filename);
        await fs.unlink(path.join(root, filename)).catch((e) => {
          if (e.code !== "ENOENT") throw e;
        });
        await query.run("DELETE FROM file_deletions WHERE filename = ?", [
          filename,
        ]);
      } catch {
        console.error(
          "Rimozione fotografia da riprovare; riferimento conservato nella coda tecnica.",
        );
      }
    }
  })().finally(() => {
