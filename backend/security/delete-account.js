...
async function deleteAccount(request, response) {
  ...
  await transaction(async (query) => {
    ...
    const notifications = await query.all(
      `SELECT b.id, b.date, a.start_time, s.name, p.user_id
    ...
    for (const item of notifications) {
      await query.run(
        ...
        [
          item.user_id,
          `La prenotazione presso ${item.name} del ${item.date} alle ${item.start_time} è stata cancellata per eliminazione dell'account organizzatore.`,
          now,
        ],
      );
    }
    const photos = await query.all(
    ...
    for (const photo of photos) {
      await query.run(
        ...
        [photoName(photo.photo_path), now],
      );
    }
    await query.run(
      `DELETE FROM bookings WHERE id IN
      (SELECT booking_id FROM booking_participants WHERE user_id = ? AND participant_role = 'organizer')`,
      [user.id],
    );
