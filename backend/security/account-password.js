...
async function checkPassword(request) {
  ...
  if (typeof password !== 'string') throw accountError(400, 'VALIDATION_ERROR', 'Inserisci la password corrente.');
  const user = await queries(getDatabase()).get('SELECT * FROM users WHERE id = ?', [request.user.id]);
  if (!user || !await verifyPassword(password, user.password_hash)) {
    throw accountError(400, 'CURRENT_PASSWORD_INVALID', 'La password corrente non è corretta.');
  }
  return user;
}

async function assertCurrent(query, request, user) {
  const current = await query.get(`SELECT u.id FROM users u JOIN auth_sessions s ON s.user_id = u.id
  ...
  if (!current) throw accountError(401, 'UNAUTHORIZED', 'La sessione non è più valida. Accedi nuovamente.');
}
...
async function changePassword(request, response) {
  try { validatePassword(request.body?.newPassword); } catch (e) { e.status = 400; throw e; }
...
module.exports = { accountError, checkPassword, assertCurrent, clearSessionCookie, changePassword };
