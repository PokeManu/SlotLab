...
function smtpConfig(env = process.env) {
  ...
  if (!env.SLOTLAB_SMTP_HOST) {
    if (
      smtpVariables.some((name) => env[name] !== undefined && env[name] !== "")
    ) {
  ...
  if (
    ...
    port < 1 ||
    port > 65535 ||
    !["true", "false"].includes(env.SLOTLAB_SMTP_SECURE || "true")
  )
    throw new Error("Configurazione SMTP non valida.");
  const from = validateEmail(env.SLOTLAB_SMTP_FROM);
  if (Boolean(env.SLOTLAB_SMTP_USER) !== Boolean(env.SLOTLAB_SMTP_PASSWORD))
    throw new Error("Credenziali SMTP incomplete.");
  return {
    from,
    transport: {
      host: env.SLOTLAB_SMTP_HOST,
      port,
      secure: (env.SLOTLAB_SMTP_SECURE || "true") === "true",
      requireTLS: true,
      connectionTimeout: 4000,
      greetingTimeout: 4000,
      socketTimeout: 4000,
      auth: env.SLOTLAB_SMTP_USER
        ? { user: env.SLOTLAB_SMTP_USER, pass: env.SLOTLAB_SMTP_PASSWORD }
        : undefined,
      tls: env.SLOTLAB_SMTP_CA_FILE
        ? { ca: fs.readFileSync(env.SLOTLAB_SMTP_CA_FILE) }
        : undefined,
  ...
  try {
    await transaction(async (query) => {
      ...
      if (result.rejected.length || !result.accepted.length)
        throw new Error("Invio SMTP rifiutato.");
    });
  } finally {
    transport.close();
  }
...
function requestRecovery(email) {
  ...
  pending.add(email);
  queue = queue
    .then(() => recoverPassword(email))
    .catch(() => {
      console.error(
        "Recupero password non completato. Verificare SMTP e database; nessun dato account viene registrato.",
      );
    })
