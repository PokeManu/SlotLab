...
async function space(name = "Fixture statistiche") {
  return (
    ...
    VALUES(?,?,0,'study_room',20,1,'active')`,
      [buildingId, name],
    )
  ).lastId;
}
...
async function booking(spaceId, availabilityId, date) {
  const id = (
    ...
    VALUES(?,?,?,'confirmed',?)`,
      [spaceId, availabilityId, date, new Date().toISOString()],
    )
  ).lastId;
  await db.run(
    `INSERT INTO booking_participants(booking_id,user_id,participant_role,present)
    VALUES(?,?,'organizer',0)`,
    [id, userId],
  );
  return id;
...
before(async () => {
  ...
  for (const role of ["admin", "user"]) {
    const row = await db.run(
      ...
      VALUES('Fixture','Punto5',?,?,?,?)`,
      [`${role}@punto5.test`, hash, role, new Date().toISOString()],
    );
    if (role === "user") userId = row.lastId;
    const result = await request(
      "/auth/login",
      "POST",
      { email: `${role}@punto5.test`, password },
      null,
    );
  ...
  buildingId = (
    await db.run(`INSERT INTO buildings(number,name,address,latitude,longitude)
    VALUES(999,'Edificio di test','Ambiente isolato',38,13)`)
  ).lastId;
});
after(async () => {
  await stopServer();
...
test("statistiche HTTP: ruoli, date rigorose, snapshot senza prenotazioni e capienza storica", async () => {
  ...
  await transaction(async (query) => {
    await query.run(
      "UPDATE occurrence_tracking SET tracking_started_at=?,consolidated_until=? WHERE id=1",
  ...
  for (const query of [
    "",
    "?from=2026-01-01&until=2026-01-02",
  ...
  assert.equal(
    (await request(`/admin/spaces/${spaceId}`, "PATCH", { capacity: 40 }))
      .status,
    200,
  );
  assert.equal((await request(url)).body.data.offeredCapacity, 20);
  const historic = await request(
    `/admin/statistics?dateFrom=2020-01-01&dateTo=2020-01-02`,
  );
...
test("nuova fascia: cancella future incompatibili ritirate con notifica e conserva idempotenza", async () => {
  ...
  const result = await request(
    `/admin/spaces/${spaceId}/availability`,
    "POST",
    {
      validFrom: date,
      validUntil: date,
  ...
  assert.equal(
    (
      await db.get(
        "SELECT booking_id FROM booking_requests WHERE idempotency_key=?",
        ["fixture-retirement"],
      )
    ).booking_id,
    null,
  ...
  assert.equal(
    (
      await db.get(
        "SELECT COUNT(*) AS n FROM notifications WHERE user_id=? AND message LIKE '%configurazione%'",
        [userId],
      )
    ).n,
    1,
  ...
  const catalog = await request(
    "/spaces?availableNow=true&minSeats=1",
    "GET",
    undefined,
    userToken,
  );
...
test("riutilizzo fascia: sostituisce la ritirata senza duplicarla e conserva le prenotazioni compatibili", async () => {
  ...
  const bookingId = await booking(spaceId, old, date);
  const result = await request(
    `/admin/spaces/${spaceId}/availability/${old}/reuse`,
    "POST",
    {
      validFrom: date,
      validUntil: date,
  ...
  assert.equal(
    (
      await db.get("SELECT availability_id AS id FROM bookings WHERE id=?", [
        bookingId,
      ])
    ).id,
    replacement,
  ...
  assert.equal(
    (
      await db.get(
        "SELECT superseded_by_id AS id FROM availabilities WHERE id=?",
        [old],
      )
    ).id,
    replacement,
  ...
  assert.deepEqual(
    visible.body.data.map((item) => item.availabilityId),
    [replacement],
  );
});
test("modifica fascia: rollback conserva prenotazioni se la notifica fallisce", async () => {
  ...
  const bookingId = await booking(spaceId, old, date);
  await db.run(
    "CREATE TRIGGER fail_notice BEFORE INSERT ON notifications BEGIN SELECT RAISE(ABORT,'fixture'); END",
  );
  try {
    const result = await request(
      `/admin/spaces/${spaceId}/availability/${old}`,
      "PATCH",
      {
        validFrom: date,
        validUntil: date,
    ...
    assert.equal(
      (await db.get("SELECT is_retired FROM availabilities WHERE id=?", [old]))
        .is_retired,
      0,
    );
  } finally {
...
test("il limite cinque conta solo prenotazioni con inizio futuro", async () => {
  ...
  const limitUser = await db.run(
    `INSERT INTO users(first_name,last_name,email,password_hash,role,created_at)
    VALUES('Limit','Five',?,?, 'user', ?)`,
    [limitEmail, await hashPassword(password), new Date().toISOString()],
  );
  const login = await request(
    "/auth/login",
    "POST",
    { email: limitEmail, password },
    null,
  );
  ...
  const today = day(0);
  const endedAvailabilityId = await availability(
    endedSpaceId,
    today,
    "00:00",
    "00:01",
  );
  const endedBooking = await db.run(
    `INSERT INTO bookings(space_id,availability_id,date,status,created_at)
    VALUES(?,?,?,'confirmed',?)`,
    [endedSpaceId, endedAvailabilityId, today, new Date().toISOString()],
  );
  await db.run(
    `INSERT INTO booking_participants(booking_id,user_id,participant_role,present)
    VALUES(?,?, 'organizer',0)`,
    [endedBooking.lastId, limitUser.lastId],
  );
  for (let offset = 2; offset <= 5; offset += 1) {
    ...
    const futureBooking = await db.run(
      `INSERT INTO bookings(space_id,availability_id,date,status,created_at)
      VALUES(?,?,?,'confirmed',?)`,
      [
        futureSpaceId,
    ...
    await db.run(
      `INSERT INTO booking_participants(booking_id,user_id,participant_role,present)
      VALUES(?,?, 'organizer',0)`,
      [futureBooking.lastId, limitUser.lastId],
    );
  }
...
test("indisponibilità retrodatata e modifica conservano prenotazione terminata e snapshot", async () => {
  ...
  const bookingId = await booking(spaceId, old, date);
  await db.run(
    `INSERT INTO slot_occurrences(space_id,date,start_time,end_time,offered_capacity,was_offered,finalized_at)
    VALUES(?,?,'10:00','12:00',20,1,?)`,
    [spaceId, date, new Date().toISOString()],
  );
  ...
  assert.equal(
    (
      await db.get(
        "SELECT offered_capacity FROM slot_occurrences WHERE space_id=?",
        [spaceId],
      )
    ).offered_capacity,
    20,
...
test("una prenotazione in corso blocca indisponibilità, disattivazione ed eliminazione senza modifiche parziali", async () => {
  const local = romeNow();
  if (local.time === "23:59") return;
  const spaceId = await space("Fascia in corso");
  const availabilityId = await availability(
    spaceId,
    local.date,
    "00:00",
    "23:59",
  );
  const bookingId = await booking(spaceId, availabilityId, local.date);
  const notificationsBefore = (
    await db.get("SELECT COUNT(*) AS count FROM notifications")
  ).count;
  const unavailable = await request(
    `/admin/spaces/${spaceId}/unavailability`,
    "POST",
    {
      date: local.date,
      startTime: "00:00",
      endTime: "23:59",
      reason: "Intervento durante la fascia",
    },
  );
  const deactivated = await request(`/admin/spaces/${spaceId}`, "PATCH", {
    status: "maintenance",
  });
  const deleted = await request(`/admin/spaces/${spaceId}`, "DELETE");
  for (const result of [unavailable, deactivated, deleted]) {
    assert.equal(result.status, 409);
    assert.equal(result.body.error.code, "BOOKING_IN_PROGRESS");
  }
  assert.equal(
    (await db.get("SELECT status FROM spaces WHERE id=?", [spaceId])).status,
    "active",
  );
  assert.ok(await db.get("SELECT id FROM bookings WHERE id=?", [bookingId]));
  assert.equal(
  ...
  assert.equal(
    (
      await db.get(
        "SELECT COUNT(*) AS count FROM unavailabilities WHERE space_id=?",
        [spaceId],
      )
    ).count,
    0,
