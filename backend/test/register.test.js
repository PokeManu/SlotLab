...
const account = {
  ...
  email: ' MARIO@Example.test ', password: 'TestPassword2026!',
};
...
test('registrazione pubblica: 201, dati normalizzati, hash verificabile e nessuna sessione', async () => {
  ...
  assert.equal((await get('SELECT COUNT(*) AS count FROM auth_sessions;')).count, 0);
});
...
test('campi mancanti, tipi errati e nomi vuoti restituiscono 400 senza inserimenti', async () => {
  ...
  const bodies = [{}, [], { ...account, firstName: ' ' }, { ...account, lastName: '\t' }];
  for (const field of ['firstName', 'lastName', 'email', 'password']) {
    const missing = { ...account };
    delete missing[field];
    bodies.push(missing, { ...account, [field]: null }, { ...account, [field]: 42 });
  }
  ...
  assert.deepEqual(await get('SELECT COUNT(*) AS count FROM users;'), beforeCount);
});
...
test('email e password non valide restituiscono i codici del contratto', async () => {
  ...
  const cases = [
    ...
    ...['', 'debole', 'Aa1!aaa', 'Aa1!' + 'a'.repeat(61), 'Testpassword!',
      'testpassword1!', 'TESTPASSWORD1!', 'Testpassword1',
      ' TestPassword2026!', 'TestPassword2026!\n'].map(password =>
      [{ email: 'invalid-password@example.test', password }, 'INVALID_PASSWORD_FORMAT']),
  ];
...
test('il campo role e sempre rifiutato, anche user o null', async () => {
  for (const role of ['admin', 'user', null, false]) {
    const { response, body } = await register({ ...account, email: 'role@example.test', role });
    assert.equal(response.status, 400);
    assert.equal(body.error.code, 'VALIDATION_ERROR');
  }
  assert.equal(await get('SELECT id FROM users WHERE email = ?;', ['role@example.test']), undefined);
});

test('email duplicata normalizzata: 409 e account originale conservato', async () => {
  const original = await get('SELECT * FROM users WHERE email = ?;', ['mario@example.test']);
  const { response, body } = await register({
    ...account, email: '  MARIO@example.TEST  ', password: 'DifferentPassword2026!',
  });
  assert.equal(response.status, 409);
  assert.equal(body.error.code, 'EMAIL_ALREADY_EXISTS');
  assert.deepEqual(await get('SELECT * FROM users WHERE id = ?;', [original.id]), original);
});
...
test('richieste contemporanee con stessa email: una creazione e un conflitto', async () => {
  ...
  assert.equal(results.find(result => result.response.status === 409).body.error.code,
    'EMAIL_ALREADY_EXISTS');
  assert.equal((await get('SELECT COUNT(*) AS count FROM users WHERE email = ?;',
    ['race@example.test'])).count, 1);
});

test('nomi con apostrofi e testo SQL sono salvati come dati', async () => {
  const firstName = "Mario'); DROP TABLE users; --";
  const { response, body } = await register({
    ...account, email: 'quoted@example.test', firstName, lastName: "D'Amico",
  });
  assert.equal(response.status, 201);
  assert.equal(body.data.firstName, firstName);
  assert.equal((await get('SELECT first_name FROM users WHERE id = ?;', [body.data.id])).first_name,
    firstName);
});
...
test('errore database: 500 generico senza esporre o registrare dettagli riservati', async (t) => {
  ...
  t.mock.method(getDatabase(), 'run', (sql, values, callback) => {
    callback(Object.assign(new Error('dettaglio database riservato'), { code: 'SQLITE_ERROR' }));
  });
  const { response, body } = await register({ ...account, email: 'failure@example.test' });
  ...
  assert.equal(await get('SELECT id FROM users WHERE email = ?;', ['failure@example.test']), undefined);
});
