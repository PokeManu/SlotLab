...
const testDirectory = fs.mkdtempSync(
  path.join(os.tmpdir(), "slotlab-backend-"),
);
...
test("le migrazioni sono ripetibili", async () => {
  ...
  assert.deepEqual(firstRun.appliedNow, [1, 2, 3, 4, 5, 6, 7, 8]);
...
test("la migrazione 2 conserva dati, vincoli e ID gia utilizzati", async (t) => {
  for (const keepRows of [true, false]) {
    const { database, filePath } = await createVersionOneDatabase(
      t,
      `upgrade-${keepRows}.sqlite`,
    );
    await execute(
      database,
      `
    ...
    await run(
      database,
      keepRows
        ? "DELETE FROM slot_occurrences WHERE id = 50;"
        : "DELETE FROM slot_occurrences;",
    );
    const originalRows = await all(
      database,
      "SELECT * FROM slot_occurrences ORDER BY id;",
    );
    assert.deepEqual(
      (await migrate({ databasePath: filePath })).appliedNow,
      [2, 3, 4, 5, 6, 7, 8],
    );
    assert.deepEqual(
      await all(database, "SELECT * FROM slot_occurrences ORDER BY id;"),
      originalRows,
    );
    assert.deepEqual(
      (await migrate({ databasePath: filePath })).appliedNow,
    ...
    await assert.rejects(
      run(database, insert, [1]),
      /UNIQUE constraint failed/,
    );
    await assert.rejects(
      run(database, insert, [999]),
...
test("dati storici incompleti annullano la migrazione 2 senza perdere dati", async (t) => {
  ...
  assert.deepEqual(
    await all(database, "SELECT version FROM schema_migrations;"),
    [{ version: 1 }],
  );
  assert.equal(
    await get(
      database,
      "SELECT name FROM sqlite_master WHERE name = 'slot_occurrences_new';",
    ),
    undefined,
  );
  await run(
    database,
    "UPDATE slot_occurrences SET offered_capacity = 20 WHERE id = 2;",
  );
  assert.deepEqual(
    (await migrate({ databasePath: filePath })).appliedNow,
    [2, 3, 4, 5, 6, 7, 8],
...
test("la connessione condivisa usa lo schema completo", async () => {
  ...
  assert.equal(queryIndexes.count, 16);
...
test("i dati iniziali non producono duplicati", async () => {
  ...
  const buildingCount = await get(
    database,
    "SELECT COUNT(*) AS count FROM buildings;",
  );
  const spaceCount = await get(
    database,
    "SELECT COUNT(*) AS count FROM spaces;",
  );
  const availabilityCount = await get(
    database,
    "SELECT COUNT(*) AS count FROM availabilities WHERE is_retired = 0;",
  );
  const spaceServiceCount = await get(
    database,
    "SELECT COUNT(*) AS count FROM space_services;",
  );
  const buildingNumbers = await all(
    database,
    "SELECT number FROM buildings ORDER BY number;",
  );
  const roomServices = await all(
    database,
    `
    SELECT sv.code
  ...
  assert.deepEqual(
    buildingNumbers.map((building) => building.number),
  ...
  assert.equal(
    await verifyPassword(testAdmin.password, admin.password_hash),
    true,
  );
  assert.equal(
    await verifyPassword("DifferentPassword2026!", admin.password_hash),
    false,
  );
});
  ...
  assert.throws(
    () =>
      selectedDatabasePath({
        ...
        SLOTLAB_DEMO_DB_PATH: operational,
      }),
...
test("il seed non promuove un utente normale", async () => {
  ...
  const user = await get(database, "SELECT role FROM users WHERE email = ?;", [
    "normal@example.test",
  ]);
...
test("Express rispetta la base del contratto HTTP", async () => {
  ...
  assert.equal(
    (await largeBodyResponse.json()).error.code,
    "PAYLOAD_TOO_LARGE",
  );
  await stopServer();
