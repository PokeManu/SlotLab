...
test('foto segnalazione: solo admin, contenuto e file mancante', async () => {
  ...
  const data = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a5WQAAAAASUVORK5CYII=', 'base64');
  fs.writeFileSync(path.join(root, filename), data);
  const user = await post('login', account);
  const admin = await post('login', { ...account, email: 'auth-admin@example.test' });
  const space = await get('SELECT id FROM spaces LIMIT 1');
  const now = new Date().toISOString();
  const reportId = await run("INSERT INTO reports(user_id,space_id,category,description,priority,status,photo_path,created_at,updated_at) VALUES(?,?,'other','Foto test','low','open',?,?,?)", [user.user.id, space.id, filename, now, now]);
  const url = `${baseUrl}/api/v1/admin/reports/${reportId}/photo`;
  try {
    assert.equal((await fetch(url)).status, 401);
    assert.equal((await fetch(url, { headers: { Authorization: `Bearer ${user.accessToken}` } })).status, 403);
    ...
    assert.equal((await fetch(userUrl, { headers: { Authorization: `Bearer ${other.accessToken}` } })).status, 404);
    fs.unlinkSync(path.join(root, filename));
    assert.equal((await fetch(url, { headers })).status, 404);
    await run('UPDATE reports SET photo_path = ? WHERE id = ?', ['../secret.png', reportId]);
...
test('avvisi globali: autorizzazione, destinatari e rollback atomico', async () => {
  const admin = await post('login', { ...account, email: 'auth-admin@example.test' });
  const user = await post('login', account);
  const publish = (token, body = { title: ' Avviso test ', message: ' Messaggio test ' }) => fetch(`${baseUrl}/api/v1/admin/announcements`, {
  ...
  const recipients = (await get("SELECT COUNT(*) AS count FROM users WHERE role = 'user'")).count;
  ...
  assert.equal((await get('SELECT author_id FROM announcements WHERE id = ?', [data.id])).author_id, admin.user.id);
  assert.equal((await get('SELECT COUNT(*) AS count FROM notifications WHERE announcement_id = ?', [data.id])).count, recipients);
  assert.equal((await get("SELECT COUNT(*) AS count FROM notifications n JOIN users u ON u.id=n.user_id WHERE n.announcement_id=? AND u.role='admin'", [data.id])).count, 0);
  const notification = await get('SELECT title, message, type, read_at FROM notifications WHERE announcement_id=? AND user_id=?', [data.id, user.user.id]);
  assert.deepEqual(notification, { title: data.title, message: data.message, type: 'global_announcement', read_at: null });
  const newcomer = await post('register', { ...account, email: 'announcement-new@example.test' });
  assert.equal((await get('SELECT COUNT(*) AS count FROM notifications WHERE user_id=?', [newcomer.id])).count, 0);
  const before = (await get('SELECT COUNT(*) AS count FROM announcements')).count;
  await run("CREATE TRIGGER fail_announcement BEFORE INSERT ON notifications WHEN NEW.type='global_announcement' BEGIN SELECT RAISE(ABORT, 'test failure'); END");
  try {
    assert.equal((await publish(admin.accessToken)).status, 500);
    assert.equal((await get('SELECT COUNT(*) AS count FROM announcements')).count, before);
    assert.equal((await get("SELECT COUNT(*) AS count FROM notifications WHERE type='global_announcement'")).count, recipients);
  } finally {
...
test('fasce ritirate e ricreate: il catalogo restituisce i nuovi ID prenotabili', async () => {
  ...
  const spaceId = await run("INSERT INTO spaces(building_id,name,floor,type,capacity,accessible,status) VALUES(?,'Test ricrea fasce',1,'study_room',24,1,'active')", [building.id]);
  const day = new Date(Date.now() + 7 * 86400000);
  const date = day.toISOString().slice(0, 10);
  const input = { validFrom: date, validUntil: date, weekday: day.getUTCDay() || 7, startTime: '08:00', endTime: '10:00' };
  ...
  const create = async body => {
    const response = await fetch(adminUrl, { method: 'POST', headers, body: JSON.stringify(body) });
    assert.equal(response.status, 201);
    return (await response.json()).data.availabilityId;
  };
  const oldId = await create(input);
  assert.equal((await fetch(`${adminUrl}/${oldId}`, { method: 'DELETE', headers })).status, 204);
  const newId = await create({ ...input, startTime: '14:00', endTime: '16:00' });
  assert.notEqual(newId, oldId);
  const response = await fetch(`${baseUrl}/api/v1/spaces/${spaceId}/availability?date=${date}`, { headers: { Authorization: `Bearer ${user.accessToken}` } });
  assert.equal(response.status, 200);
  const { data } = await response.json();
...
test('prenotazioni admin: elenco reale con organizzatore e paginazione', async () => {
  ...
  const bookingId = await run("INSERT INTO bookings(space_id,availability_id,date,status,created_at) VALUES(?,?,'2026-09-14','confirmed',?)", [slot.space_id,slot.id,new Date().toISOString()]);
  await run("INSERT INTO booking_participants(booking_id,user_id,participant_role) VALUES(?,?,'organizer')", [bookingId,user.user.id]);
  ...
  assert.equal(response.status,200);
  const body = await response.json();
  assert.equal(body.pagination.totalElements,(await get('SELECT COUNT(*) AS count FROM bookings')).count);
...
test('check-in sceglie la fascia corrente anche con prenotazioni precedenti', async () => {
  ...
  const admin = await post('login', { ...account, email: 'auth-admin@example.test' });
  const building = await get('SELECT id FROM buildings LIMIT 1');
  const spaceId = await run("INSERT INTO spaces(building_id,name,floor,type,capacity,accessible,status) VALUES(?,'Test check-in multiplo',1,'study_room',24,1,'active')", [building.id]);
  const parts = Object.fromEntries(new Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Rome', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).formatToParts(new Date()).map(p => [p.type,p.value]));
  const date = `${parts.year}-${parts.month}-${parts.day}`;
  // Due minuti prima: la prova resta stabile se cambia il minuto durante le richieste.
  const minute = Math.max(0, Number(parts.hour) * 60 + Number(parts.minute) - 2);
  const start = `${String(Math.floor(minute / 60)).padStart(2,'0')}:${String(minute % 60).padStart(2,'0')}`;
  const slot = await run("INSERT INTO availabilities(space_id,valid_from,valid_until,weekday,start_time,end_time,is_retired) VALUES(?,'2000-01-01','2099-12-31',1,?,'23:59',0)", [spaceId,start]);
  const insert = async dateValue => {
    const id = await run("INSERT INTO bookings(space_id,availability_id,date,status,created_at) VALUES(?,?,?,'confirmed',?)", [spaceId,slot,dateValue,new Date().toISOString()]);
    await run("INSERT INTO booking_participants(booking_id,user_id,participant_role) VALUES(?,?,'organizer')", [id,user.user.id]);
    return id;
  };
  const previous = await insert('2000-01-01');
  const current = await insert(date);
  const url = `${baseUrl}/api/v1/spaces/${spaceId}/check-in`;
  const verify = token => fetch(url,{method:'POST',headers:token?{Authorization:`Bearer ${token}`}:{}});
  ...
  assert.equal((await get('SELECT present FROM booking_participants WHERE booking_id=?',[previous])).present,0);
  await run('UPDATE booking_participants SET present=1,checked_in_at=? WHERE booking_id=?', ['2000-01-01T08:00:00.000Z',previous]);
  ...
  await run('UPDATE booking_participants SET present=0,checked_in_at=NULL WHERE booking_id=?',[previous]);
...
test('prenotazioni: capienza condivisa con fasce ritirate, idempotenza e notifiche', async () => {
  ...
  const building = await get('SELECT id FROM buildings LIMIT 1');
  const spaceId = await run("INSERT INTO spaces(building_id,name,floor,type,capacity,accessible,status) VALUES(?,'Capienza test',1,'study_room',2,1,'active')",[building.id]);
  const day = new Date(Date.now()+7*86400000), date=day.toISOString().slice(0,10);
  const weekday = day.getUTCDay() || 7;
  const makeSlot = retired => run('INSERT INTO availabilities(space_id,valid_from,valid_until,weekday,start_time,end_time,is_retired) VALUES(?,?,?,?,?,?,?)',[spaceId,date,date,weekday,'10:00','12:00',retired]);
  const oldSlot = await makeSlot(1), newSlot = await makeSlot(0);
  const oldBooking = await run("INSERT INTO bookings(space_id,availability_id,date,status,created_at) VALUES(?,?,?,'confirmed',?)",[spaceId,oldSlot,date,new Date().toISOString()]);
  await run("INSERT INTO booking_participants(booking_id,user_id,participant_role) VALUES(?,?,'organizer')",[oldBooking,accounts[0].user.id]);
  const headers = index => ({Authorization:`Bearer ${accounts[index].accessToken}`,'Content-Type':'application/json'});
  const body = {spaceId,availabilityId:newSlot,date,participantEmails:[]};
  const create = (index,key) => fetch(`${baseUrl}/api/v1/bookings`,{method:'POST',headers:{...headers(index),'Idempotency-Key':key},body:JSON.stringify(body)});
  const availability = await fetch(`${baseUrl}/api/v1/spaces/${spaceId}/availability?date=${date}`,{headers:headers(1)});
  assert.equal((await availability.json()).data[0].availableSeats,1);
  const responses = await Promise.all([create(1,'capacity-one'),create(2,'capacity-two')]);
  assert.deepEqual(responses.map(r=>r.status).sort(),[201,409]);
  const winner = responses[0].status===201 ? 1:2, loser=winner===1?2:1;
  const created = (await responses[winner-1].json()).data;
  assert.equal((await responses[loser-1].json()).error.code,'INSUFFICIENT_CAPACITY');
  const repeat = await create(winner,winner===1?'capacity-one':'capacity-two');
  assert.equal(repeat.status,200);
  assert.equal((await repeat.json()).data.id,created.id);
  assert.equal((await get("SELECT COUNT(*) AS count FROM notifications WHERE user_id=? AND type='booking_created'",[accounts[winner].user.id])).count,1);
  const addition = await fetch(`${baseUrl}/api/v1/bookings/${created.id}/participants`,{method:'POST',headers:headers(winner),body:JSON.stringify({email:`capacity-${loser}@example.test`})});
  assert.equal(addition.status,409);
  assert.equal((await addition.json()).error.code,'INSUFFICIENT_CAPACITY');
  const removed = await fetch(`${baseUrl}/api/v1/bookings/${created.id}`,{method:'DELETE',headers:headers(winner)});
  assert.equal(removed.status,204);
  assert.equal((await create(winner,winner===1?'capacity-one':'capacity-two')).status,409);
});
...
test('admin: modifica spazio e servizi, cancellazione con foto e notifiche mirate', async () => {
  ...
  const user=await post('login',account);
  const headers={Authorization:`Bearer ${admin.accessToken}`,'Content-Type':'application/json'};
  const request=async(method,url,body)=>fetch(`${baseUrl}/api/v1/admin${url}`,{method,headers,body:body?JSON.stringify(body):undefined});
  const building=await get('SELECT id FROM buildings LIMIT 1');
  const created=await request('POST','/spaces',{buildingId:building.id,name:'Spazio eliminabile',floor:1,type:'study_room',capacity:10,accessible:true,status:'active',serviceCodes:['wifi']});
  assert.equal(created.status,201);
  const id=(await created.json()).data.id;
  const newBuilding=await run("INSERT INTO buildings(number,name,address,latitude,longitude) VALUES(998,'Edificio modifica','Via test',38,13)");
  assert.equal((await request('PATCH',`/spaces/${id}`,{buildingId:newBuilding,serviceCodes:['computer','projector']})).status,200);
  ...
  const photoRoot=path.join(directory,'delete-photos'); fs.mkdirSync(photoRoot);
  process.env.SLOTLAB_UPLOAD_DIR=photoRoot;
  fs.writeFileSync(path.join(photoRoot,'delete-test.png'),'test');
  const now=new Date().toISOString();
  await run("INSERT INTO reports(user_id,space_id,category,description,priority,status,photo_path,created_at,updated_at) VALUES(?,?,'other','Test eliminazione','low','open','delete-test.png',?,?)",[user.user.id,id,now,now]);
  const slot=await run("INSERT INTO availabilities(space_id,valid_from,valid_until,weekday,start_time,end_time,is_retired) VALUES(?,'2026-01-01','2099-12-31',1,'10:00','12:00',0)",[id]);
  const booking=await run("INSERT INTO bookings(space_id,availability_id,date,status,created_at) VALUES(?,?,'2099-12-01','confirmed',?)",[id,slot,now]);
  await run("INSERT INTO booking_participants(booking_id,user_id,participant_role) VALUES(?,?,'organizer')",[booking,user.user.id]);
  const before=(await get("SELECT COUNT(*) AS count FROM notifications WHERE user_id=? AND type='space_unavailable'",[user.user.id])).count;
  try {
    ...
    assert.equal((await get('SELECT COUNT(*) AS count FROM bookings WHERE id=?',[booking])).count,0);
    assert.equal((await get("SELECT COUNT(*) AS count FROM notifications WHERE user_id=? AND type='space_unavailable'",[user.user.id])).count,before+1);
  } finally { delete process.env.SLOTLAB_UPLOAD_DIR; }
});
