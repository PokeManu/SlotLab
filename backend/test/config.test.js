...
test("la configurazione rifiuta segreti mancanti o malformati", () => {
  for (const secret of [undefined, "", "password", "x".repeat(64)]) {
    assert.throws(
      () => readConfig({ SLOTLAB_JWT_SECRET: secret }),
      /SLOTLAB_JWT_SECRET/,
    );
  }
});
test("il proxy attendibile richiede una scelta esplicita e limitata al loopback", () => {
  ...
  assert.equal(
    readConfig(testEnvironment({ SLOTLAB_TRUST_PROXY: "loopback" })).trustProxy,
    "loopback",
  );
  for (const value of ["true", "*", "1"]) {
    assert.throws(
      () => readConfig(testEnvironment({ SLOTLAB_TRUST_PROXY: value })),
      /SLOTLAB_TRUST_PROXY/,
    );
  }
});
test("la configurazione valida ambiente e porta", () => {
  ...
  assert.equal(
    readConfig(testEnvironment({ PORT: "0", NODE_ENV: "test" })).port,
    0,
  );
  assert.equal(readConfig(testEnvironment({ PORT: "3100" })).port, 3100);
  assert.throws(
    () => readConfig(testEnvironment({ NODE_ENV: "prod" })),
    /NODE_ENV/,
  );
});
