...
async function createFixture() {
  ...
  await new Promise((resolve, reject) =>
    connection.exec(schema, (error) => (error ? reject(error) : resolve())),
  );
  ...
  await db.run(
    "INSERT INTO buildings (id, number, name, address, latitude, longitude) VALUES (1, 1, ?, ?, 38, 13);",
    ["Edificio A", "Viale delle Scienze"],
  );
  ...
  for (let id = 1; id <= 12; id += 1) {
    await db.run(
      ...
      VALUES (?, ?, 'User', ?, 'hash', 'user', '2026-01-01T00:00:00.000Z');`,
      [id, `User${id}`, `user${id}@example.test`],
    );
  }
  ...
  for (let id = 1; id <= 12; id += 1) {
    ...
    await db.run(
      `INSERT INTO booking_participants (booking_id, user_id, participant_role, present, checked_in_at)
      VALUES (?, ?, ?, ?, ?);`,
      [
        bookingId,
        id,
        id === (bookingId === 1 ? 1 : bookingId === 2 ? 4 : 8)
          ? "organizer"
          : "participant",
        id <= 9 ? 1 : 0,
...
test("statistiche: coorte di snapshot, gruppi distinti, ritirate, tassi e segnalazioni", async (t) => {
  ...
  assert.deepEqual(result.usage, [
    { type: "study_room", presences: 9, percentage: 100 },
  ]);
  assert.deepEqual(result.mostBookedSpaces, [
    {
      spaceId: 1,
      spaceName: "Aula 1",
      bookings: 3,
  ...
  assert.deepEqual(result.reportsByCategory, [
    { category: "other", count: 1 },
    { category: "technical", count: 1 },
  ]);
  assert.deepEqual(result.reportsByStatus, [
    { status: "open", count: 1 },
...
test("rapporti storici: 20 posti con tre gruppi, fascia vuota e marker nello stesso giorno", async (t) => {
  ...
  const twoDays = await statistics(
    fixture.db,
    "2026-09-10",
    "2026-09-11",
  ...
  await fixture.db.run(
    "UPDATE booking_participants SET present=1, checked_in_at='2026-09-10T08:30:00.000Z' WHERE user_id=10;",
  );
  const tenPresent = await statistics(
    fixture.db,
    "2026-09-10",
    "2026-09-11",
    new Date("2026-09-11T10:00:00.000Z"),
  ...
  const beforeMarker = await statistics(
    fixture.db,
    "2026-09-10",
    "2026-09-10",
    new Date("2026-09-10T15:30:00.000Z"),
...
test("statistiche: future, in corso, indisponibili e booking senza snapshot non entrano nella coorte", async (t) => {
  ...
  t.after(() => closeDatabase(fixture.connection));
  await fixture.db.run(
    "UPDATE slot_occurrences SET offered_capacity=NULL, was_offered=NULL, finalized_at=NULL WHERE id=3;",
  );
  await fixture.db
    .run(`INSERT INTO bookings (id, space_id, availability_id, date, status, created_at)
...
test("periodo statistiche: date reali, obbligatorie e nessun campo legacy o extra", () => {
  assert.deepEqual(
    validateStatisticsPeriod({ dateFrom: "2026-02-01", dateTo: "2026-02-28" }),
    {
      dateFrom: "2026-02-01",
      dateTo: "2026-02-28",
    },
  ...
  for (const query of [
    ...
    assert.throws(
      () => validateStatisticsPeriod(query),
