...
const account = {
  firstName: 'Refresh', lastName: 'Test', email: 'refresh@example.test', password: 'RefreshPassword2026!',
};
let baseUrl;
...
function refresh(value, headers = {}) {
  return post('refresh', {}, { 'X-SlotLab-Request': '1', ...(value ? { Cookie: value } : {}), ...headers });
}
...
function run(sql, values = []) {
  return new Promise((resolve, reject) => {
    getDatabase().run(sql, values, error => error ? reject(error) : resolve());
  });
}

async function me(token) {
  return fetch(`${baseUrl}/api/v1/users/me`, { headers: { Authorization: `Bearer ${token}` } });
}
...
test('refresh senza Bearer: ruota cookie e jti, revoca vecchio access e conserva data login', async () => {
  ...
  assert.deepEqual(Object.keys(result.body.data).sort(), ['accessToken', 'tokenType', 'expiresIn'].sort());
  ...
  const claims = jwt.verify(result.body.data.accessToken, key, { algorithms: ['HS256'] });
  const session = await getSession();
  assert.equal(session.access_token_jti, claims.jti);
  assert.equal(session.refresh_token_hash, hashRefreshToken(cookie(result).split('=')[1]));
  assert.equal(session.created_at, previous.created_at);
  assert.ok(Math.abs(Date.parse(session.refresh_expires_at) - Date.now() - 604800000) < 2000);
  for (const attribute of ['HttpOnly', 'Secure', 'SameSite=Lax', 'Path=/api/v1/auth', 'Max-Age=604800']) {
    assert.ok(result.response.headers.get('set-cookie').split('; ').includes(attribute));
  }
...
test('cookie mancante, malformato e sconosciuto: errori distinti, nessuna rotazione', async () => {
  ...
  const previous = await getSession();
  for (const [value, code] of [[undefined, 'REFRESH_TOKEN_MISSING'],
    ['slotlab_refresh=', 'REFRESH_TOKEN_INVALID'], ['slotlab_refresh=%ZZ', 'REFRESH_TOKEN_INVALID'],
    ['slotlab_refresh=j:null', 'REFRESH_TOKEN_INVALID'],
    [`slotlab_refresh=${crypto.randomBytes(32).toString('base64url')}`, 'REFRESH_TOKEN_INVALID']]) {
...
test('refresh scaduto: 401 REFRESH_TOKEN_EXPIRED senza alterare la sessione', async () => {
  const login = await post('login', account);
  await run('UPDATE auth_sessions SET refresh_expires_at = ?;', [new Date(Date.now() - 1000).toISOString()]);
  const previous = await getSession();
  const result = await refresh(cookie(login));
  assert.equal(result.response.status, 401);
...
test('header di protezione obbligatorio e richieste cross-site rifiutate', async () => {
  const login = await post('login', account);
  const previous = await getSession();
  for (const headers of [{ Cookie: cookie(login) },
    { Cookie: cookie(login), 'X-SlotLab-Request': '1', 'Sec-Fetch-Site': 'cross-site' }]) {
  ...
  const preflight = await fetch(`${baseUrl}/api/v1/auth/refresh`, {
    method: 'OPTIONS', headers: { Origin: 'https://outside.example',
      'Access-Control-Request-Method': 'POST', 'Access-Control-Request-Headers': 'x-slotlab-request' },
  });
...
test('due refresh letti insieme: una sola rotazione e SESSION_REPLACED per il secondo', async (t) => {
  ...
  t.mock.method(database, 'get', (sql, values, callback) => {
    if (!sql.includes('WHERE s.refresh_token_hash')) return originalGet(sql, values, callback);
  ...
  const results = await Promise.all([refresh(cookie(login)), refresh(cookie(login))]);
  ...
  assert.equal((await getSession()).refresh_token_hash, hashRefreshToken(cookie(winner).split('=')[1]));
...
test('nuovo login rende inutilizzabile il refresh precedente', async () => {
  ...
  const previous = await getSession();
  assert.equal((await refresh(cookie(first))).body.error.code, 'REFRESH_TOKEN_INVALID');
  assert.deepEqual(await getSession(), previous);
