...
test("sceglie la prima occorrenza per un orario ambiguo e consente la seconda esplicita", () => {
  ...
  assert.equal(
    localDateTimeToInstant("2026-10-25", "02:30", {
...
test("l’anticipo minimo di un’ora attraversa la mezzanotte e il cambio DST", () => {
  assert.equal(
    isAtLeastAhead(
      ...
      "00:30",
      3600,
      new Date("2026-09-10T21:30:00.000Z"),
    ),
    true,
  );
  assert.equal(
    isAtLeastAhead(
      ...
      "00:30",
      3600,
      new Date("2026-09-10T21:30:01.000Z"),
    ),
    false,
  );
  assert.equal(
...
test("il termine prenotazioni richiede più di un’ora e blocca esattamente T-60", () => {
  const start = localDateTimeToInstant("2026-09-11", "10:00");
  assert.equal(
    isMoreThanAhead(
      "2026-09-11",
      "10:00",
      3600,
      new Date(start.getTime() - 3600 * 1000 - 1),
    ),
    true,
  );
  assert.equal(
...
test("la finestra di check-in usa secondi reali e include i due estremi", () => {
  ...
  assert.equal(checkInWindow("2026-09-10", "10:00", start), null);
  assert.equal(
    checkInWindow(
      "2026-09-10",
      "10:00",
  ...
  assert.equal(
    checkInWindow(
      "2026-09-10",
      "10:00",
      new Date(start.getTime() - 15 * 60000 - 1),
    ),
    "early",
  );
  assert.equal(
    checkInWindow(
      "2026-09-10",
      "10:00",
      new Date(start.getTime() + 30 * 60000 + 1),
    ),
    "expired",
  );
});
