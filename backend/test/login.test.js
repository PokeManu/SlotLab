...
function claimsFor(result) {
  return jwt.verify(result.body.data.accessToken, key, {
    algorithms: ["HS256"],
  });
}
before(async () => {
  ...
  await seed({
    admin: { ...account, email: "admin-login@example.test" },
...
test("login pubblico: JWT firmato con soli claim previsti e durata di 30 minuti", async () => {
  ...
  assert.throws(
    () =>
      jwt.verify(data.accessToken, key, {
        algorithms: ["HS256"],
        clockTimestamp: claims.exp,
      }),
    { name: "TokenExpiredError" },
  );
  assert.throws(() =>
    jwt.verify(data.accessToken, crypto.randomBytes(32), {
      algorithms: ["HS256"],
    }),
  );
});
test("refresh: cookie protetto di 7 giorni e nel database soltanto hash e jti", async () => {
  ...
  assert.equal(
    Date.parse(session.refresh_expires_at) - Date.parse(session.created_at),
    604800000,
  );
  assert.equal(JSON.stringify(session).includes(refresh), false);
  assert.equal(JSON.stringify(result.body).includes(refresh), false);
  assert.equal(
    JSON.stringify(session).includes(result.body.data.accessToken),
    false,
  );
});
test("email assente, password errata e input non valido: stesso 401 senza cambiare sessione", async () => {
  ...
  for (const input of [
    { email: "missing@example.test", password: account.password },
    { ...account, password: "WrongPassword2026!" },
  ...
  assert.deepEqual(
    await get("SELECT COUNT(*) AS count FROM auth_sessions;"),
    beforeCount,
  );
});
...
test("login concorrenti: una sola coppia hash refresh e jti corrente", async () => {
  ...
  const current = results.filter(
    (result) => claimsFor(result).jti === session.access_token_jti,
  );
...
test("fallimento salvataggio sessione: niente token/cookie e sessione precedente conservata", async (t) => {
  ...
  t.mock.method(getDatabase(), "run", (sql, values, callback) => {
    callback(
      Object.assign(new Error("dettaglio SQL riservato"), {
        code: "SQLITE_ERROR",
      }),
    );
  });
  ...
  t.mock.method(database, "get", (sql, values, callback) => {
    return originalGet(sql, values, (error, row) => {
      ...
      database.run(
        ...
        ["test-only-changed-hash", row.id],
        (changeError) => callback(changeError, row),
      );
    });
  });
  const result = await post("login", input);
